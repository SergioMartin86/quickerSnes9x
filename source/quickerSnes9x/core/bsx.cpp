...
static __thread const uint8 flashcard[20] = {0x4D, 0x00, 0x50, 0x00, // vendor id
                                             0x00, 0x00,             // ?
...
static void BSX_Map_LoROM(void)
{
  ...
  for (c = 0; c < 0x400; c += 16)
  {
    for (i = c; i < c + 8; i++) Map[i + 0x400] = Map[i + 0xC00] = &MapROM[(c << 11) % FlashSize];

    for (i = c + 8; i < c + 16; i++) Map[i + 0x400] = Map[i + 0xC00] = &MapROM[(c << 11) % FlashSize] - 0x8000;
...
static void map_psram_mirror_sub(uint32 bank)
{
  ...
  } else
  {
    for (c = 0; c < 0x100; c += 16)
    {
      if ((bank & 0x7F) >= 0x40)
      {
        for (i = c; i < c + 8; i++) Map[i + bank] = &PSRAM[(c << 11) % PSRAM_SIZE];
      ...
      for (i = c + 8; i < c + 16; i++) Map[i + bank] = &PSRAM[(c << 11) % PSRAM_SIZE] - 0x8000;
...
static void BSX_Map_PSRAM(void)
{
  ...
  if (!BSX.MMC[0x02])
  {
    ...
    if (!BSX.MMC[0x05] && !BSX.MMC[0x06])
    {
      ...
      if (BSX.MMC[0x03]) map_psram_mirror_sub(0x00);

      if (BSX.MMC[0x04]) map_psram_mirror_sub(0x80);
    } else if (BSX.MMC[0x05] && !BSX.MMC[0x06])
    {
      //Map PSRAM to 20-2F/A0-AF
      if (BSX.MMC[0x03]) map_psram_mirror_sub(0x20);

      if (BSX.MMC[0x04]) map_psram_mirror_sub(0xA0);
    } else if (!BSX.MMC[0x05] && BSX.MMC[0x06])
    {
      //Map PSRAM to 40-4F/C0-CF
      if (BSX.MMC[0x03]) map_psram_mirror_sub(0x40);

      if (BSX.MMC[0x04]) map_psram_mirror_sub(0xC0);
    } else
    {
      //Map PSRAM to 60-6F/E0-EF
      if (BSX.MMC[0x03]) map_psram_mirror_sub(0x60);

      if (BSX.MMC[0x04]) map_psram_mirror_sub(0xE0);
    }

    //Map PSRAM to 70-7D/F0-FF
    if (BSX.MMC[0x03]) map_psram_mirror_sub(0x70);

    if (BSX.MMC[0x04]) map_psram_mirror_sub(0xF0);
  } else
  {
    //HiROM Mode
    if (!BSX.MMC[0x05] && !BSX.MMC[0x06])
    ...
    } else if (BSX.MMC[0x05] && !BSX.MMC[0x06])
    {
      ...
      if (BSX.MMC[0x03]) map_psram_mirror_sub(0x50);

      if (BSX.MMC[0x04]) map_psram_mirror_sub(0xD0);
    } else if (!BSX.MMC[0x05] && BSX.MMC[0x06])
    {
...
uint8 S9xGetBSX(uint32 address)
{
  ...
  // MMC
  if ((bank >= 0x01 && bank <= 0x0E)) return (BSX.MMC[bank]);
  ...
  if (bank >= 0xC0)
  {
    ...
    case 0x0002:
    case 0x8002:
      if (BSX.flash_bsr) t = 0xC0; // Page Status Register
      break;

    case 0x0004:
    case 0x8004:
      if (BSX.flash_gsr) t = 0x82; // Global Status Register
      break;

    case 0x5555:
      if (BSX.flash_enable) t = 0x80; // ???
      break;
    ...
    case 0xFF12:
      ...
      if (BSX.read_enable) t = flashcard[offset - 0xFF00];
      break;
    }
...
uint8 S9xBSXGetRTC(void)
{
  ...
  if (BSX.out_index > 22) BSX.out_index = 0;
...
uint8 S9xGetBSXPPU(uint16 address)
{
  ...
  case 0x2188: t = BSX.PPU[0x2188 - BSXPPUBASE]; break;

  // Logical Channel 2 (R/W) [6bit]
  case 0x2189: t = BSX.PPU[0x2189 - BSXPPUBASE]; break;
  ...
  case 0x218B:
    ...
    {
      if (BSX.PPU[0x2188 - BSXPPUBASE] == 0 && BSX.PPU[0x2189 - BSXPPUBASE] == 0) { BSX.PPU[0x218B - BSXPPUBASE] = 0x90; }
  ...
  //Stream 2
  // Logical Channel 1 + Data Structure (R/W)
  case 0x218E: t = BSX.PPU[0x218E - BSXPPUBASE]; break;

  // Logical Channel 2 (R/W) [6bit]
  case 0x218F: t = BSX.PPU[0x218F - BSXPPUBASE]; break;
  ...
  case 0x2191:
    ...
    {
      if (BSX.PPU[0x218E - BSXPPUBASE] == 0 && BSX.PPU[0x218F - BSXPPUBASE] == 0) { BSX.PPU[0x2191 - BSXPPUBASE] = 0x90; }
  ...
  case 0x2194: t = BSX.PPU[0x2194 - BSXPPUBASE]; break;

  // Unknown
  case 0x2195: t = BSX.PPU[0x2195 - BSXPPUBASE]; break;

  // Satellaview Status (R)
  case 0x2196: t = BSX.PPU[0x2196 - BSXPPUBASE]; break;

  // Soundlink Settings (R/W)
  case 0x2197: t = BSX.PPU[0x2197 - BSXPPUBASE]; break;

  // Serial I/O - Serial Number (R/W)
  case 0x2198: t = BSX.PPU[0x2198 - BSXPPUBASE]; break;

  // Serial I/O - Unknown (R/W)
  case 0x2199: t = BSX.PPU[0x2199 - BSXPPUBASE]; break;

  default: t = OpenBus; break;
  }
...
void S9xSetBSXPPU(uint8 byte, uint16 address)
{
  ...
  // Logical Channel 1 + Data Structure (R/W)
  case 0x2188:
    if (BSX.PPU[0x2188 - BSXPPUBASE] == byte) { BSX.sat_stream1_count = 0; }
    BSX.PPU[0x2188 - BSXPPUBASE] = byte;
    break;

  // Logical Channel 2 (R/W) [6bit]
  case 0x2189:
    if (BSX.PPU[0x2188 - BSXPPUBASE] == (byte & 0x3F)) { BSX.sat_stream1_count = 0; }
    BSX.PPU[0x2189 - BSXPPUBASE] = byte & 0x3F;
    break;

  // Prefix Latch (R/W)
  case 0x218B: BSX.sat_pf_latch1_enable = (byte != 0); break;

  // Data Latch (R/W)
  case 0x218C:
    if (BSX.PPU[0x2188 - BSXPPUBASE] == 0 && BSX.PPU[0x2189 - BSXPPUBASE] == 0) { BSX.out_index = 0; }
  ...
  // Logical Channel 1 + Data Structure (R/W)
  case 0x218E:
    if (BSX.PPU[0x218E - BSXPPUBASE] == byte) { BSX.sat_stream2_count = 0; }
    BSX.PPU[0x218E - BSXPPUBASE] = byte;
    break;

  // Logical Channel 2 (R/W) [6bit]
  case 0x218F:
    if (BSX.PPU[0x218F - BSXPPUBASE] == (byte & 0x3F)) { BSX.sat_stream2_count = 0; }
    BSX.PPU[0x218F - BSXPPUBASE] = byte & 0x3F;
    break;

  // Prefix Latch (R/W)
  case 0x2191: BSX.sat_pf_latch2_enable = (byte != 0); break;

  // Data Latch (R/W)
  case 0x2192:
    if (BSX.PPU[0x218E - BSXPPUBASE] == 0 && BSX.PPU[0x218F - BSXPPUBASE] == 0) { BSX.out_index = 0; }
    BSX.sat_dt_latch2_enable = (byte != 0);
    break;

  //Other
  // Satellaview LED / Stream Enable (R/W) [4bit]
  case 0x2194: BSX.PPU[0x2194 - BSXPPUBASE] = byte & 0x0F; break;

  // Soundlink Settings (R/W)
  case 0x2197: BSX.PPU[0x2197 - BSXPPUBASE] = byte; break;
  }
}

uint8 *S9xGetBasePointerBSX(uint32 address) { return (MapROM); }
...
#ifdef BSX_DEBUG
      for (int i = 0; i <= 0x1F; i++) printf("BS: ROM Header %02X: %02X\n", i, header[i]);
...
{
  if (Settings.BSXItself) memset(Memory.ROM, 0, FLASH_SIZE);
  ...
  if (BSX.sat_stream2.is_open()) BSX.sat_stream2.close();

  if (Settings.BS) BSX_Map();
}
...
static int is_bsx(unsigned char *p)
{
  if ((p[26] == 0x33 || p[26] == 0xFF) && (!p[21] || (p[21] & 131) == 128) && valid_normal_bank(p[24]))
  {
    ...
    if ((m == 0xFF && p[23] == 0xFF) || (!(m & 0xF) && ((m >> 4) - 1 < 12))) return (1);
  }
