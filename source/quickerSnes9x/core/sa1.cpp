...
static void S9xSetSA1MemMap(uint32 which1, uint8 map)
{
  ...
  for (int c = 0; c < 0x100; c += 16)
  {
    ...
    for (int i = c; i < c + 16; i++) Memory.Map[start + i] = SA1.Map[start + i] = block;
  }
  ...
  for (int c = 0; c < 0x200; c += 16)
  {
    ...
    for (int i = c + 8; i < c + 16; i++) Memory.Map[start2 + i] = SA1.Map[start2 + i] = block;
  }
}
...
uint8 S9xGetSA1(uint32 address)
{
  ...
  case 0x230d: // variable-length data read port (H)
  {
    ...
    if (Memory.FillRAM[0x2258] & 0x80) S9xSA1ReadVariableLengthData(TRUE, FALSE);
...
void S9xSetSA1(uint8 byte, uint32 address)
{
  ...
  case 0x2200: // SA-1 control
#ifdef DEBUGGER
    if (byte & 0x60) printf("SA-1 sleep\n");
#endif
    ...
    if (byte & 0x80)
    {
      ...
      if (Memory.FillRAM[0x220a] & 0x80) Memory.FillRAM[0x220b] &= ~0x80;
    }
    ...
    if (byte & 0x10)
    {
      ...
      if (Memory.FillRAM[0x220a] & 0x10) Memory.FillRAM[0x220b] &= ~0x10;
    }
  ...
  case 0x2202: // S-CPU interrupt clear
    // S-CPU IRQ clear
    if (byte & 0x80) Memory.FillRAM[0x2300] &= ~0x80;

    // S-CPU CHDMA IRQ clear
    if (byte & 0x20) Memory.FillRAM[0x2300] &= ~0x20;

    if (!(Memory.FillRAM[0x2300] & 0xa0)) CPU.IRQExternal = FALSE;
  ...
  case 0x220a: // SA-1 interrupt enable
    // SA-1 IRQ enable
    if (((byte ^ Memory.FillRAM[0x220a]) & 0x80) && (Memory.FillRAM[0x2301] & byte & 0x80)) Memory.FillRAM[0x220b] &= ~0x80;

    // SA-1 timer IRQ enable
    if (((byte ^ Memory.FillRAM[0x220a]) & 0x40) && (Memory.FillRAM[0x2301] & byte & 0x40)) Memory.FillRAM[0x220b] &= ~0x40;

    // SA-1 DMA IRQ enable
    if (((byte ^ Memory.FillRAM[0x220a]) & 0x20) && (Memory.FillRAM[0x2301] & byte & 0x20)) Memory.FillRAM[0x220b] &= ~0x20;

    // SA-1 NMI enable
    if (((byte ^ Memory.FillRAM[0x220a]) & 0x10) && (Memory.FillRAM[0x2301] & byte & 0x10)) Memory.FillRAM[0x220b] &= ~0x10;

    break;

  case 0x220b: // SA-1 interrupt clear
    // SA-1 IRQ clear
    if (byte & 0x80) Memory.FillRAM[0x2301] &= ~0x80;

    // SA-1 timer IRQ clear
    if (byte & 0x40) Memory.FillRAM[0x2301] &= ~0x40;

    // SA-1 DMA IRQ clear
    if (byte & 0x20) Memory.FillRAM[0x2301] &= ~0x20;

    // SA-1 NMI clear
    if (byte & 0x10) Memory.FillRAM[0x2301] &= ~0x10;
  ...
  case 0x2225: // SA-1 BW-RAM mapping
    if (byte != Memory.FillRAM[0x2225]) S9xSA1SetBWRAMMemMap(byte);
  ...
  case 0x2231: // character conversion DMA parameters
    ...
    if (byte & 0x80) SA1.in_char_dma = FALSE;
  ...
  case 0x2236: // DMA destination start address (LH)
    ...
    if ((Memory.FillRAM[0x2230] & 0xa4) == 0x80) // Normal DMA to I-RAM
      S9xSA1DMA();
    else if ((Memory.FillRAM[0x2230] & 0xb0) == 0xb0) // CC1
    {
  ...
  case 0x224f: // bitmap register F
    ...
    if ((Memory.FillRAM[0x2230] & 0xb0) == 0xa0) // CC2
    {
      ...
      if ((SA1.in_char_dma & 3) == 0) S9xSA1CharConv2();
    }
  ...
  if (address >= 0x2200 && address <= 0x22ff) Memory.FillRAM[address] = byte;
}
...
uint8 S9xSA1GetByte(uint32 address)
{
  ...
  {
  case CMemory::MAP_PPU: return (S9xGetSA1(address & 0xffff));

  case CMemory::MAP_LOROM_SRAM:
  case CMemory::MAP_SA1RAM: return (*(Memory.SRAM + (address & 0xffff)));

  case CMemory::MAP_BWRAM: return (*(SA1.BWRAM + ((address & 0x7fff) - 0x6000)));
...
uint16 S9xSA1GetWord(uint32 address, s9xwrap_t w)
{
  ...
  case WRAP_NONE:
  default: return (SA1OpenBus | (S9xSA1GetByte(address + 1) << 8));
  }
}
...
void S9xSA1SetByte(uint8 byte, uint32 address)
{
  ...
  {
  case CMemory::MAP_PPU: S9xSetSA1(byte, address & 0xffff); return;

  case CMemory::MAP_LOROM_SRAM:
  case CMemory::MAP_SA1RAM: *(Memory.SRAM + (address & 0xffff)) = byte; return;

  case CMemory::MAP_BWRAM: *(SA1.BWRAM + ((address & 0x7fff) - 0x6000)) = byte; return;
...
void S9xSA1SetWord(uint16 Word, uint32 address, enum s9xwrap_t w, enum s9xwriteorder_t o)
{
  ...
  case WRAP_NONE:
  default: S9xSA1SetByte(Word >> 8, address + 1); break;
  }
...
void S9xSA1SetPCBase(uint32 address)
{
  ...
  case CMemory::MAP_BWRAM: SA1.PCBase = SA1.BWRAM - 0x6000 - (address & 0x8000); return;

  case CMemory::MAP_SA1RAM: SA1.PCBase = Memory.SRAM; return;

  default: SA1.PCBase = NULL; return;
  }
}
