...
extern __thread int         HDMA_ModeByteCounts[8];
extern thread_local SPC7110 s7emu;

static __thread uint8 sdd1_decode_buffer[0x10000];
...
bool8 S9xDoDMA(uint8 Channel)
{
  ...
  case 0x18:
  case 0x19:
    if (IPPU.RenderThisFrame) FLUSH_REDRAW();
    break;
  }
  ...
  if (Settings.SPC7110)
  {
    if (d->AAddress == 0x4800 || d->ABank == 0x50)
    {
      ...
      for (int i = 0; i < d->TransferBytes; i++) spc7110_dma[i] = s7emu.decomp.read();
  ...
  {
    sprintf(String,
            "DMA[%d]: %s Mode:%d 0x%02X%04X->0x21%02X Bytes:%d (%s) V:%03d",
            Channel,
            d->ReverseTransfer ? "PPU->CPU" : "CPU->PPU",
            d->TransferMode,
            d->ABank,
            d->AAddress,
            d->BAddress,
    ...
    if (d->BAddress == 0x18 || d->BAddress == 0x19 || d->BAddress == 0x39 || d->BAddress == 0x3a)
      sprintf(String, "%s VRAM: %04X (%d,%d) %s", String, PPU.VMA.Address, PPU.VMA.Increment, PPU.VMA.FullGraphicCount, PPU.VMA.High ? "word" : "byte");
  ...
  if (!d->ReverseTransfer)
  {
    ...
    inWRAM_DMA = ((!in_sa1_dma && !in_sdd1_dma && !spc7110_dma) && (d->ABank == 0x7e || d->ABank == 0x7f || (!(d->ABank & 0x40) && d->AAddress < 0x2000)));
...
#define UPDATE_COUNTERS                                                                                                                                                            \
  ...
  if (CPU.NMILine && (Timings.NMITriggerPos != 0xffff))
  {
    ...
    if (Timings.NMITriggerPos >= Timings.H_Max) Timings.NMITriggerPos -= Timings.H_Max;
  }

  // Release the memory used in SPC7110 DMA
  if (Settings.SPC7110)
  {
    if (spc7110_dma) delete[] spc7110_dma;
  }
...
void S9xStartHDMA(void)
{
  ...
  // XXX: Not quite right...
  if (PPU.HDMA != 0) ADD_CYCLES(Timings.DMACPUSync);
...
uint8 S9xDoHDMA(uint8 byte)
{
  ...
  for (uint8 mask = 1; mask; mask <<= 1, p++, d++)
  {
    if (byte & mask)
    {
      ...
      if (!HDMAMemPointers[d]) HDMAMemPointers[d] = S9xGetMemPointer(ShiftedIBank + IAddr);
      ...
      if (p->DoTransfer)
      {
        ...
        {
          sprintf(String,
                  "H-DMA[%d] %s (%d) 0x%06X->0x21%02X %s, Count: %3d, Rep: %s, V-LINE: %3ld %02X%04X",
                  p - DMA,
                  p->ReverseTransfer ? "read" : "write",
                  p->TransferMode,
                  ShiftedIBank + IAddr,
                  p->BAddress,
