...
void S9xLoadConfigFiles()
{
  ...
  if (Settings.ForceLoROM) Settings.ForceHiROM = false;

  Settings.ForcePAL  = conf.GetBool("ROM::PAL", false);
  Settings.ForceNTSC = conf.GetBool("ROM::NTSC", false);
  if (Settings.ForcePAL) Settings.ForceNTSC = false;
  ...
  if (conf.Exists("Settings::FrameTime")) Settings.FrameTimePAL = Settings.FrameTimeNTSC = conf.GetUInt("Settings::FrameTime", 16667);
  ...
  if (conf.Exists("Controls::Port1")) parse_controller_spec(0, conf.GetString("Controls::Port1"));
  if (conf.Exists("Controls::Port2")) parse_controller_spec(1, conf.GetString("Controls::Port2"));
  ...
  if (conf.Exists("Netplay::Port")) Settings.Port = -(int)conf.GetUInt("Netplay::Port");

  Settings.ServerName[0] = '\0';
  if (conf.Exists("Netplay::Server")) conf.GetString("Netplay::Server", Settings.ServerName, 128);
#endif
...
static bool parse_controller_spec(int port, const char *arg)
{
  ...
  else if (!strncasecmp(arg, "mp5:", 4) && ((arg[4] >= '1' && arg[4] <= '8') || arg[4] == 'n') && ((arg[5] >= '1' && arg[5] <= '8') || arg[5] == 'n') &&
           ((arg[6] >= '1' && arg[6] <= '8') || arg[6] == 'n') && ((arg[7] >= '1' && arg[7] <= '8') || arg[7] == 'n') && arg[8] == '\0')
    S9xSetController(
      port, CTL_MP5, (arg[4] == 'n') ? -1 : arg[4] - '1', (arg[5] == 'n') ? -1 : arg[5] - '1', (arg[6] == 'n') ? -1 : arg[6] - '1', (arg[7] == 'n') ? -1 : arg[7] - '1');
  else
    return (false);
  ...
  for (int i = 1; i < argc; i++)
  {
    if (*argv[i] == '-')
    {
      if (!strcasecmp(argv[i], "-help"))
        ...
        else if (!strcasecmp(argv[i], "-playbackrate"))
        {
          if (i + 1 < argc)
          {
            Settings.SoundPlaybackRate = atoi(argv[++i]);
            if (Settings.SoundPlaybackRate < 8192) Settings.SoundPlaybackRate = 8192;
          } else
            S9xUsage();
        } else if (!strcasecmp(argv[i], "-inputrate"))
        {
          if (i + 1 < argc)
          {
            Settings.SoundInputRate = atoi(argv[++i]);
            if (Settings.SoundInputRate < 8192) Settings.SoundInputRate = 8192;
          } else
            S9xUsage();
        } else if (!strcasecmp(argv[i], "-reversestereo"))
          Settings.ReverseStereo = TRUE;
        else if (!strcasecmp(argv[i], "-nostereo"))
          Settings.Stereo = FALSE;
        else if (!strcasecmp(argv[i], "-eightbit"))
          Settings.SixteenBitSound = FALSE;
        else if (!strcasecmp(argv[i], "-mute"))
          Settings.Mute = TRUE;
        else
          ...
          if (!strcasecmp(argv[i], "-displayframerate"))
          ...
          else if (!strcasecmp(argv[i], "-displaykeypress"))
            Settings.DisplayPressedKeys = TRUE;
          else if (!strcasecmp(argv[i], "-nohires"))
            Settings.SupportHiRes = FALSE;
          else if (!strcasecmp(argv[i], "-notransparency"))
            Settings.Transparency = FALSE;
          else if (!strcasecmp(argv[i], "-nowindows"))
            ...
            else if (!strcasecmp(argv[i], "-nomouse"))
              Settings.MouseMaster = FALSE;
            else if (!strcasecmp(argv[i], "-nosuperscope"))
              Settings.SuperScopeMaster = FALSE;
            else if (!strcasecmp(argv[i], "-nojustifier"))
              Settings.JustifierMaster = FALSE;
            else if (!strcasecmp(argv[i], "-port1") || !strcasecmp(argv[i], "-port2"))
            {
              if (i + 1 < argc)
              {
                i++;
                if (!parse_controller_spec(argv[i - 1][5] - '1', argv[i])) S9xUsage();
              } else
                S9xUsage();
            } else
              ...
              if (!strcasecmp(argv[i], "-hirom"))
                ...
                else if (!strcasecmp(argv[i], "-cheat"))
                  Settings.ApplyCheats = TRUE;
                else if (!strcasecmp(argv[i], "-gamegenie"))
                {
                  if (i + 1 < argc)
                  {
                    uint32      address;
                    ...
                    S9xUsage();
                } else if (!strcasecmp(argv[i], "-actionreplay"))
                {
                  if (i + 1 < argc)
                  {
                    uint32      address;
                    ...
                    S9xUsage();
                } else if (!strcasecmp(argv[i], "-goldfinger"))
                {
                  if (i + 1 < argc)
                  {
                    uint32      address;
                    ...
                    {
                      for (int c = 0; c < num_bytes; c++) S9xAddCheat(TRUE, FALSE, address + c, bytes[c]);
                    } else
                  ...
                  if (!strcasecmp(argv[i], "-hdmatiming"))
                {
                  ...
                  {
                    int p = atoi(argv[++i]);
                    if (p > 0 && p < 200) Settings.HDMATimingHack = p;
                  } else
                    S9xUsage();
                } else if (!strcasecmp(argv[i], "-invalidvramaccess"))
                  ...
                  if (!strcasecmp(argv[i], "-frameskip"))
                  {
                    if (i + 1 < argc)
                      ...
                      S9xUsage();
                  } else if (!strcasecmp(argv[i], "-frametime"))
                  {
                    if (i + 1 < argc)
                      Settings.FrameTimePAL = Settings.FrameTimeNTSC = atoi(argv[++i]);
                    else
                      S9xUsage();
                  } else if (!strcasecmp(argv[i], "-upanddown"))
                    Settings.UpAndDown = TRUE;
                  else if (!strcasecmp(argv[i], "-conf"))
                  {
                    if (++i >= argc) S9xUsage();
                    // Else do nothing, S9xLoadConfigFiles() handled it.
                  } else if (!strcasecmp(argv[i], "-nostdconf"))
                  {
