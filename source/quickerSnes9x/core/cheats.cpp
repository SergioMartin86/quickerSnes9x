...
{
  for (int i = 0; i < len; i++)
    if ((code[i] < '0' || code[i] > '9') && (code[i] < 'a' || code[i] > 'f') && (code[i] < 'A' || code[i] > 'F')) return (FALSE);
...
const char *S9xProActionReplayToRaw(const char *code, uint32 &address, uint8 &byte)
{
  ...
  if (strlen(code) != 8 || !S9xAllHex(code, 8) || sscanf(code, "%x", &data) != 1) return ("Invalid Pro Action Replay code - should be 8 hex digits in length.");
...
const char *S9xGoldFingerToRaw(const char *code, uint32 &address, bool8 &sram, uint8 &num_bytes, uint8 bytes[3])
{
  ...
  if (strlen(code) != 14) return ("Invalid Gold Finger code - should be 14 hex digits in length.");

  strncpy(tmp, code, 5);
  tmp[5] = 0;
  if (sscanf(tmp, "%x", &address) != 1) return ("Invalid Gold Finger code.");
...
const char *S9xGameGenieToRaw(const char *code, uint32 &address, uint8 &byte)
{
  ...
  if (strlen(code) != 9 || *(code + 4) != '-' || !S9xAllHex(code, 4) || !S9xAllHex(code + 5, 4)) return ("Invalid Game Genie(tm) code - should be 'xxxx-xxxx'.");
  ...
  for (int i = 2; i < 10; i++)
  {
    if (islower(new_code[i])) new_code[i] = toupper(new_code[i]);
    ...
    if (j == 16) return ("Invalid hex-character in Game Genie(tm) code.");
  }
  ...
  address = ((address & 0x003c00) << 10) + ((address & 0x00003c) << 14) + ((address & 0xf00000) >> 8) + ((address & 0x000003) << 10) + ((address & 0x00c000) >> 6) +
            ((address & 0x0f0000) >> 12) + ((address & 0x0003c0) >> 6);
...
void S9xSearchForChange(SCheatData *d, S9xCheatComparisonType cmp, S9xCheatDataSize size, bool8 is_signed, bool8 update)
{
  ...
  if (is_signed)
  {
    ...
    for (i = 0; i < 0x2000 - l; i++)
    {
      ...
      {
        if (update) d->CIRAM[i] = d->FillRAM[i + 0x3000];
      } else
  ...
  for (i = 0x20000 - l; i < 0x20000; i++) BIT_CLEAR(d->WRAM_BITS, i);

  for (i = 0x10000 - l; i < 0x10000; i++) BIT_CLEAR(d->SRAM_BITS, i);
}
...
void S9xOutputCheatSearchResults(SCheatData *d)
{
  ...
  for (i = 0; i < 0x20000; i++)
  {
    if (TEST_BIT(d->WRAM_BITS, i)) printf("WRAM: %05x: %02x\n", i, d->RAM[i]);
  }

  for (i = 0; i < 0x10000; i++)
  {
    if (TEST_BIT(d->SRAM_BITS, i)) printf("SRAM: %04x: %02x\n", i, d->SRAM[i]);
  }

  for (i = 0; i < 0x2000; i++)
  {
    if (TEST_BIT(d->IRAM_BITS, i)) printf("IRAM: %05x: %02x\n", i, d->FillRAM[i + 0x3000]);
  }
}
