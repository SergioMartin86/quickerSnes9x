...
extern __thread uint16      BlackColourMap[256];
extern __thread uint16      DirectColourMaps[8][256];
extern __thread uint8       mul_brightness[16][32];
extern __thread struct SBG  BG;
extern __thread struct SGFX GFX;
...
#define COLOR_ADD(C1, C2)                                                                                                                                                          \
  (GFX.X2[((((C1)&RGB_REMOVE_LOW_BITS_MASK) + ((C2)&RGB_REMOVE_LOW_BITS_MASK)) >> 1) + ((C1) & (C2)&RGB_LOW_BITS_MASK)] | (((C1) ^ (C2)) & RGB_LOW_BITS_MASK))
