...
public:
    ...
    ConfigEntry(const std::string k) { parse_key(k); }

    ConfigEntry(const std::string k, const std::string &v)
      : line(-1)
      , val(v)
    {
      parse_key(k);
...
public:
    ...
    void DeleteSection(const std::string section) { sections.erase(section); }
  };
