...
void S9xInitTileRenderer(void)
{
  ...
  for (i = 0; i < 16; i++)
  {
    ...
    for (uint8 bitshift = 0; bitshift < 8; bitshift++) pixbit[bitshift][i] = b << bitshift;
  }
...
static uint8 ConvertTile8(uint8 *pCache, uint32 TileAddr, uint32)
{
  ...
  #define DOBIT(n, i)                                                                                                                                                              \
    if ((pix = hrbit_odd[*(tp1 + (n))])) p1 |= pixbit[(i)][pix];                                                                                                                   \
    if ((pix = hrbit_odd[*(tp2 + (n))])) p2 |= pixbit[(i)][pix];
...
static uint8 ConvertTile4h_odd(uint8 *pCache, uint32 TileAddr, uint32 Tile)
{
  ...
  #define DOBIT(n, i)                                                                                                                                                              \
    if ((pix = hrbit_even[*(tp1 + (n))])) p1 |= pixbit[(i)][pix];                                                                                                                  \
    if ((pix = hrbit_even[*(tp2 + (n))])) p2 |= pixbit[(i)][pix];
...
/*****************************************************************************/
  ...
  /*****************************************************************************/
    ...
    #define IS_BLANK_TILE() (((Tile & H_FLIP) ? BG.BufferedFlip[TileNumber] : BG.Buffered[TileNumber]) == BLANK_TILE)
  ...
  // The backdrop is always depth = 1, so Z1 = Z2 = 1. And backdrop is always color 0.
    ...
    #define DRAW_TILE()                                                                                                                                                            \
      ...
      {                                                                                                                                                                            \
        for (x = Left; x < Right; x++) DRAW_PIXEL(x, 1);                                                                                                                           \
      }
        ...
        {                                                                                                                                                                          \
          ...
          {                                                                                                                                                                        \
            ...
            {                                                                                                                                                                      \
              ...
              {                                                                                                                                                                    \
                for (int32 w = x + HMosaic - 1; w >= x; w--) DRAW_PIXEL(w + h * GFX.PPL, (w >= (int32)Left && w < (int32)Right));                                                  \
              }                                                                                                                                                                    \
            }                                                                                                                                                                      \
          }                                                                                                                                                                        \
        } else                                                                                                                                                                     \
...
static void (*MAKENAME(Renderers_, NAME1, NAME2)[7])(ARGS) = {MAKENAME(NAME1, _, NAME2),
