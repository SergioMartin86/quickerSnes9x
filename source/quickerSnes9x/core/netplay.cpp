...
bool8 S9xNPConnectToServer(const char *hostname, int port, const char *rom_name)
{
  ...
  NetPlay.Socket             = 0;
  if (NetPlay.ServerHostName) free((char *)NetPlay.ServerHostName);
  NetPlay.ServerHostName = strdup(hostname);
  if (NetPlay.ROMName) free((char *)NetPlay.ROMName);
  NetPlay.ROMName          = strdup(rom_name);
  NetPlay.Port             = port;
  NetPlay.PendingWait4Sync = FALSE;

  #ifdef __WIN32__
  if (GUI.ClientSemaphore == NULL) GUI.ClientSemaphore = CreateSemaphore(NULL, 0, NP_JOYPAD_HIST_SIZE, NULL);

  if (NetPlay.ReplyEvent == NULL) NetPlay.ReplyEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
...
on the remote machine on this port?");
  ...
  *ptr++ = NP_CLNT_HELLO;
  ...
  if (!S9xNPGetData(NetPlay.Socket, header, 7) || header[0] != NP_SERV_MAGIC || header[1] != 0 || (header[2] & 0x1f) != NP_SERV_HELLO)
  {
...
version of the protocol. Disconnecting.");
  ...
  if (!(header[2] & 0x80) && strcmp((char *)data + 4 + 2, NetPlay.ROMName) != 0)
  {
  ...
  return (S9xNPSendReady((header[2] & 0x80) ? NP_CLNT_WAITING_FOR_ROM_IMAGE : NP_CLNT_READY));
}
  ...
  if (S9xNPConnect())
  {
    ...
    while (NetPlay.Connected)
    {
      if (S9xNPWaitForHeartBeat())
      {
        ...
        {
          if (!NetPlay.Waiting4EmulationThread && prev == (int)NetPlay.MaxBehindFrameCount)
          {
...
{
  if (!S9xNPCheckForHeartBeat(time_msec)) return FALSE;
...
bool8 S9xNPWaitForHeartBeat()
{
  ...
  while (S9xNPGetData(NetPlay.Socket, header, 3 + 4))
  {
    ...
    if ((header[2] & 0x1f) == NP_SERV_JOYPAD)
    {
      ...
      for (i = 0; i < num; i++) NetPlay.Joypads[NetPlay.JoypadWriteInd][i] = READ_LONG(&header[3 + 4 + i * sizeof(uint32)]);

      for (i = 0; i < NP_MAX_CLIENTS; i++) NetPlay.JoypadsReady[NetPlay.JoypadWriteInd][i] = TRUE;
  ...
  #endif
        S9xNPDiscardHeartbeats();
        if (S9xNPLoadROM(len - 7)) S9xNPSendReady(NP_CLNT_LOADED_ROM);
        break;
  ...
  #endif
        S9xNPDiscardHeartbeats();
        if (S9xNPGetROMImage(len - 7)) S9xNPSendReady(NP_CLNT_RECEIVED_ROM_IMAGE);
        break;
...
bool8 S9xNPGetROMImage(uint32 len)
{
  ...
  if (filename_len > PATH_MAX || !S9xNPGetData(NetPlay.Socket, (uint8 *)Memory.ROMFilename, filename_len))
  {
...
uint32 S9xNPGetJoypad(int which1)
{
  ...
  {
  #ifdef NP_DEBUG
    if (!NetPlay.JoypadsReady[NetPlay.JoypadReadInd][which1]) { S9xNPSetWarning("Missing input from server!"); }
  #endif
...
void S9xNPResetJoypadReadPos()
{
  ...
  for (int h = 0; h < NP_JOYPAD_HIST_SIZE; h++) memset((void *)&NetPlay.Joypads[h], 0, sizeof(NetPlay.Joypads[0]));
  for (int h = 0; h < NP_JOYPAD_HIST_SIZE; h++) memset((void *)&NetPlay.JoypadsReady[h], 0, sizeof(NetPlay.JoypadsReady[0]));
}
...
bool8 S9xNPGetData(int socket, uint8 *data, int length)
{
  ...
  do {
    ...
    if (!Settings.NetPlayServer && length > 1024)
    {
      NetPlay.PercentageComplete = (uint8)(((length - len) * 100) / length);
  #ifdef __WIN32__
      PostMessage(GUI.hWnd, WM_USER, NetPlay.PercentageComplete, NetPlay.PercentageComplete);
      Sleep(0);
  #endif
    }
  }
