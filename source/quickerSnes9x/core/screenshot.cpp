...
bool8 S9xDoScreenshot(int width, int height)
{
  ...
  {
    if (width > SNES_WIDTH && height <= SNES_HEIGHT_EXTENDED) imgheight = height << 1;
  } else if (Settings.StretchScreenshots == 2)
  {
    if (width <= SNES_WIDTH) imgwidth = width << 1;
  ...
  for (int y = 0; y < height; y++, screen += GFX.RealPPL)
  {
    ...
    if (imgheight != height) png_write_row(png_ptr, row_pointer);
  }
