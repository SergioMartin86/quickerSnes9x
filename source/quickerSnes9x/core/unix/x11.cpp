...
const char *S9xParseDisplayConfig(ConfigFile &conf, int pass) { return ("Unix/X11"); }

void FatalError(const char *str) {}

int ErrorHandler(Display *display, XErrorEvent *event) { return (0); }

  #ifdef USE_XVIDEO
int get_inv_shift(uint32 mask, int bpp) { return (0); }
...
const char *S9xSelectFilename(const char *def, const char *dir1, const char *ext1, const char *title) { return (NULL); }
...
char *S9xGetDisplayCommandName(s9xcommand_t cmd) { return (strdup("None")); }

void S9xHandleDisplayCommand(s9xcommand_t cmd, int16 data1, int16 data2) { return; }

bool8 S9xMapDisplayInput(const char *n, s9xcommand_t *cmd) { return (false); }

bool S9xDisplayPollButton(uint32 id, bool *pressed) { return (false); }

bool S9xDisplayPollAxis(uint32 id, int16 *value) { return (false); }

bool S9xDisplayPollPointer(uint32 id, int16 *x, int16 *y) { return (true); }

void S9xSetPalette(void) { return; }
...
void S9xParseDisplayArg(char **argv, int &i, int argc)
{
  if (!strcasecmp(argv[i], "-setrepeat"))
    ...
    GUI.use_xvideo = TRUE;
  else if (!strcasecmp(argv[i], "-maxaspect"))
    GUI.maxaspect = TRUE;
  else
  #endif
...
const char *S9xParseDisplayConfig(ConfigFile &conf, int pass)
{
  ...
  if (conf.Exists("Unix/X11::VideoMode"))
  {
    ...
    if (GUI.video_mode < 1 || GUI.video_mode > 8) GUI.video_mode = VIDEOMODE_BLOCKY;
  } else
...
bool8 SetupXvideo()
{
  ...
  ret = XvQueryExtension(GUI.display, &p_version, &p_release, &p_request_base, &p_event_base, &p_error_base);
  if (ret != Success)
  {
    fprintf(stderr, "XvQueryExtension error\n");
    return FALSE;
  }
  ...
  for (unsigned int i = minAdaptor; i < maxAdaptor && GUI.xv_port < 0; i++)
  {
    ...
    for (unsigned int j = 0; j < encodings; j++)
    {
      ...
      if (ei[j].width >= SNES_WIDTH * 2 && ei[j].height >= SNES_HEIGHT_EXTENDED * 2)
      {
        can_fit = TRUE;
        break;
      }
  ...
  for (int i = 0; i < formats; i++)
  {
    if (fo[i].id == 0x3 || fo[i].type == XvRGB)
    {
      if (fo[i].bits_per_pixel < GUI.xv_bpp)
      {
        ...
        if (fo[i].red_mask == 0x001f && fo[i].blue_mask == 0x7c00)
        {
...
void S9xInitDisplay(int argc, char **argv)
{
  ...
  if (GUI.display == NULL) FatalError("Failed to connect to X server.");
  ...
  if (!count) FatalError("Your X Window System server is unwell!");

  GUI.depth = matches[0].depth;
  if ((GUI.depth != 15 && GUI.depth != 16 && GUI.depth != 24) || (matches[0].c_class != TrueColor)) FatalError("Requiers 15, 16, 24 or 32-bit color depth supporting TrueColor.");
  ...
  if (GUI.depth == 16 && GUI.green_size == 63) GUI.green_shift++;
  ...
  if (GUI.fullscreen == TRUE)
  {
    ...
    GUI.window = XCreateWindow(GUI.display,
                               RootWindowOfScreen(GUI.screen),
                               0,
                               0,
                               WidthOfScreen(GUI.screen),
  ...
  } else
  {
    ...
    GUI.window = XCreateWindow(GUI.display,
                               RootWindowOfScreen(GUI.screen),
                               (WidthOfScreen(GUI.screen) - SNES_WIDTH * 2) / 2,
                               (HeightOfScreen(GUI.screen) - SNES_HEIGHT_EXTENDED * 2) / 2,
                               SNES_WIDTH * 2,
  ...
  #ifdef USE_XVIDEO
  if (GUI.use_xvideo) { GUI.use_xvideo = SetupXvideo(); }
  #endif
  ...
  case 16:
    if (GUI.red_size != GUI.green_size || GUI.blue_size != GUI.green_size)
    {
      ...
      else if (GUI.red_shift > GUI.blue_shift)
  ...
  {
  default:
  case 32: GUI.bytes_per_pixel = 4; break;
  ...
  case 15:
  case 16: GUI.bytes_per_pixel = 2; break;
  }
...
void S9xDeinitDisplay(void)
{
  ...
  {
  #ifdef USE_XVIDEO
    if (GUI.use_xvideo) { XvUngrabPort(GUI.display, GUI.xv_port, CurrentTime); }
  #endif
    S9xTextMode();
...
void SetupImage(void)
{
  ...
  #ifdef USE_XVIDEO
  if (GUI.use_xvideo) SetupXvImage();
  if (!GUI.use_xvideo)
  #endif
    SetupXImage();
  ...
  if (!GUI.filter_buffer) FatalError("Failed to allocate GUI.filter_buffer.");
...
void SetupXImage(void)
{
  ...
  //		fprintf(stderr, "use_shared_memory failed, switching to XPutImage.\n");
    ...
    if (!GUI.image->ximage || !GUI.image->ximage->data) FatalError("XCreateImage failed.");
...
void TakedownXImage(void)
{
  if (GUI.image->ximage)
  {
    ...
    if (GUI.use_shared_memory)
    {
      ...
      XDestroyImage(GUI.image->ximage);
      if (GUI.sm_info.shmaddr) shmdt(GUI.sm_info.shmaddr);
      if (GUI.sm_info.shmid >= 0) shmctl(GUI.sm_info.shmid, IPC_RMID, 0);
      GUI.image->ximage = NULL;
    } else
  #endif
    {
...
void S9xPutImage(int width, int height)
{
  ...
  if (GUI.video_mode == VIDEOMODE_BLOCKY || GUI.video_mode == VIDEOMODE_TV || GUI.video_mode == VIDEOMODE_SMOOTH)
    if ((width <= SNES_WIDTH) && ((prevWidth != width) || (prevHeight != height))) S9xBlitClearDelta();
  ...
  if (height < prevHeight)
  {
    ...
    for (int y = SNES_HEIGHT * 2; y < SNES_HEIGHT_EXTENDED * 2; y++)
    {
      ...
      for (int x = 0; x < p; x++) *d++ = 0;
    }
  }

  #ifdef USE_XVIDEO
  // Adjust source blit region if SNES would only fill half the screen.
  if (height <= SNES_HEIGHT_EXTENDED) GUI.imageHeight = height * 2;
  ...
  if (GUI.use_xvideo && (GUI.xv_format == FOURCC_YUY2))
  {
    ...
    for (int y = 0; y < copyHeight; y++)
    {
      for (int x = 0; x < SNES_WIDTH * 2; x += 2)
      {
        ...
        unsigned short rgb1 = (*s & 0x7FFF);
        s++;
        unsigned short rgb2 = (*s & 0x7FFF);
        s++;
...
void Repaint(bool8 isFrameBoundry)
{
  ...
  if (GUI.use_xvideo)
  {
    ...
    {
      XvShmPutImage(
        GUI.display, GUI.xv_port, GUI.window, GUI.gc, GUI.image->xvimage, 0, 0, SNES_WIDTH * 2, GUI.imageHeight, GUI.x_offset, GUI.y_offset, GUI.scale_w, GUI.scale_h, False);
    } else
    #endif
      XvPutImage(GUI.display, GUI.xv_port, GUI.window, GUI.gc, GUI.image->xvimage, 0, 0, SNES_WIDTH * 2, GUI.imageHeight, GUI.x_offset, GUI.y_offset, GUI.scale_w, GUI.scale_h);
  } else
  #endif
  #ifdef MITSHM
    if (GUI.use_shared_memory)
  ...
  if (Settings.DumpStreams && isFrameBoundry) S9xVideoLogger(GUI.image->data, SNES_WIDTH * 2, SNES_HEIGHT_EXTENDED * 2, GUI.bytes_per_pixel, GUI.image->bytes_per_line);
}

void S9xTextMode(void) { SetXRepeat(TRUE); }

void S9xGraphicsMode(void) { SetXRepeat(FALSE); }
...
void S9xProcessEvents(bool8 block)
{
  ...
  while (block || CheckForPendingXEvents(GUI.display))
  {
    ...
    {
    case KeyPress:
    case KeyRelease: S9xReportButton(((event.xkey.state & (ShiftMask | Mod1Mask | ControlMask | Mod4Mask)) << 8) | event.xkey.keycode, event.type == KeyPress); break;
    ...
    case ButtonPress:
    case ButtonRelease: S9xReportButton(0x40000000 | (event.xbutton.button - 1), event.type == ButtonPress); break;

    case Expose: Repaint(FALSE); break;
    }
  }
}
...
const char *S9xStringInput(const char *message)
{
  ...
  if (fgets(buffer, sizeof(buffer) - 2, stdin)) return (buffer);
