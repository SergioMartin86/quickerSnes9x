...
static thread_local const char dirNames[13][32] = {"",           // DEFAULT_DIR
                                                   "",           // HOME_DIR
                                                   "",           // ROMFILENAME_DIR
                                                   "rom",        // ROM_DIR
                                                   "sram",       // SRAM_DIR
...
void S9xParseArg(char **argv, int &i, int argc)
{
  ...
  else if (!strncasecmp(argv[i], "-paddev", 7) && argv[i][7] >= '1' && argv[i][7] <= '8' && argv[i][8] == '\0')
  {
    int j = argv[i][7] - '1';
    ...
    if (!strcasecmp(argv[i], "-buffersize"))
  {
    if (i + 1 < argc)
      ...
      S9xUsage();
  } else if (!strcasecmp(argv[i], "-fragmentsize"))
  {
    if (i + 1 < argc)
      unixSettings.SoundFragmentSize = atoi(argv[++i]);
    else
      S9xUsage();
  } else if (!strcasecmp(argv[i], "-sounddev"))
  {
    if (i + 1 < argc)
      sound_device = argv[++i];
    else
  ...
  else if (!strcasecmp(argv[i], "-rwbuffersize"))
  {
    if (i + 1 < argc)
      unixSettings.rewindBufferSize = atoi(argv[++i]);
    else
      S9xUsage();
  } else if (!strcasecmp(argv[i], "-rwgranularity"))
  {
    if (i + 1 < argc)
...
void S9xParsePortConfig(ConfigFile &conf, int pass)
{
  ...
  for (ConfigFile::secvec_t::iterator c = sec.begin(); c != sec.end(); c++) keymaps.push_back(*c);
}

static int make_snes9x_dirs(void)
{
  if (strlen(s9x_base_dir) + 1 + sizeof(dirNames[0]) > PATH_MAX + 1) return (-1);
...
const char *S9xGetFilename(const char *ex, enum s9x_getdirtype dirtype) { return (nullptr); }

const char *S9xGetFilenameInc(const char *ex, enum s9x_getdirtype dirtype) { return (nullptr); }

const char *S9xBasename(const char *f) { return (nullptr); }
...
bool8 S9xOpenSnapshotFile(const char *filename, bool8 read_only, STREAM *file) { return false; }
...
void S9xAutoSaveSRAM(void) { Memory.SaveSRAM(S9xGetFilename(".srm", SRAM_DIR)); }
...
bool8 S9xMapInput(const char *n, s9xcommand_t *cmd)
{
  ...
  if (!strncmp(n, "PseudoPointer", 13) && n[13] >= '1' && n[13] <= '8' && n[14] == '\0') return (S9xMapPointer(PseudoPointerBase + (n[13] - '1'), *cmd, false));
...
bool S9xPollButton(uint32 id, bool *pressed) { return (S9xDisplayPollButton(id, pressed)); }

bool S9xPollAxis(uint32 id, int16 *value) { return (S9xDisplayPollAxis(id, value)); }

bool S9xPollPointer(uint32 id, int16 *x, int16 *y) { return (S9xDisplayPollPointer(id, x, y)); }
...
char *S9xGetPortCommandName(s9xcommand_t cmd)
{
  ...
  case S9xButtonPort:
    ...
    case 2: return (strdup("Rewind"));
    }

    break;

  case S9xAxisPort: break;

  case S9xPointerPort: break;
  }
...
void S9xHandlePortCommand(s9xcommand_t cmd, int16 data1, int16 data2)
{
  ...
  case S9xButtonPort:
    ...
    case 1:
      if (data1) js_mod[cmd.port[2]] ^= cmd.port[3];
      break;

    case 2: rewinding = (bool8)data1; break;
    }
...
static void InitJoysticks(void)
{
  ...
  for (int i = 1; i < 8; i++) js_fd[i] = open(js_device[i], O_RDONLY | O_NONBLOCK);
...
static void InitTimer(void)
{
  ...
  if (setitimer(ITIMER_REAL, &timeout, NULL) < 0) perror("setitimer");
#endif
}
...
bool8 S9xOpenSoundDevice(void)
{
  ...
  if (ioctl(so.sound_fd, SNDCTL_DSP_SETFRAGMENT, &J) == -1) return (FALSE);

  J = K = Settings.SixteenBitSound ? AFMT_S16_NE : AFMT_U8;
  if (ioctl(so.sound_fd, SNDCTL_DSP_SETFMT, &J) == -1 || J != K) return (FALSE);

  J = K = Settings.Stereo ? 1 : 0;
  if (ioctl(so.sound_fd, SNDCTL_DSP_STEREO, &J) == -1 || J != K) return (FALSE);

  J = K = Settings.SoundPlaybackRate;
  if (ioctl(so.sound_fd, SNDCTL_DSP_SPEED, &J) == -1 || J != K) return (FALSE);

  J = 0;
  if (ioctl(so.sound_fd, SNDCTL_DSP_GETBLKSIZE, &J) == -1) return (FALSE);
...
static void *S9xProcessSound(void *)
{
  ...
  audio_buf_info info;
  if (!unixSettings.ThreadSound && (ioctl(so.sound_fd, SNDCTL_DSP_GETOSPACE, &info) == -1 || info.bytes < (int)so.fragment_size)) return (NULL);
  ...
  do {
    ...
    if (Settings.SixteenBitSound) sample_count >>= 1;
    ...
    if (Settings.SixteenBitSound) bytes_to_write <<= 1;
  ...
  #ifdef USE_THREADS
    if (unixSettings.ThreadSound) pthread_mutex_unlock(&mutex);
  #endif
    ...
    for (;;)
    {
      ...
      if (byte_offset + I > SOUND_BUFFER_SIZE) I = SOUND_BUFFER_SIZE - byte_offset;
      if (I == 0) break;
...
int __MAIN(int argc, char **argv)
{
  ...
  if (Settings.Multi) { loaded = Memory.LoadMultiCart(Settings.CartAName, Settings.CartBName); }
  ...
  if (play_smv_filename)
  {
    ...
    if (S9xMovieOpen(play_smv_filename, TRUE) != SUCCESS) exit(1);
    CPU.Flags |= flags;
  } else if (record_smv_filename)
  {
    uint32 flags = CPU.Flags & (DEBUG_MODE_FLAG | TRACE_FLAG);
    if (S9xMovieCreate(record_smv_filename, 0xFF, MOVIE_OPT_FROM_RESET, NULL, 0) != SUCCESS) exit(1);
    CPU.Flags |= flags;
  } else
  {
    if (unixSettings.rewindBufferSize) { stateMan.init(unixSettings.rewindBufferSize * 1024 * 1024); }
  }
  ...
  while (1)
  {
    ...
    if (NP_Activated)
    {
      ...
      for (int J = 0; J < 8; J++) old_joypads[J] = MovieGetJoypad(J);

      for (int J = 0; J < 8; J++) MovieSetJoypad(J, joypads[J]);
    ...
    if (NP_Activated)
    {
      for (int J = 0; J < 8; J++) MovieSetJoypad(J, old_joypads[J]);
    }
#endif
...
#ifdef JOYSTICK_SUPPORT
    if (unixSettings.JoystickEnabled && (JoypadSkip++ & 1) == 0) ReadJoysticks();
#endif
...
int initSnes9x(int argc, char **argv)
{
  ...
  if (Settings.Port < 0) Settings.Port = -Settings.Port;
