...
extern __thread struct SCheatData      Cheat;
extern __thread struct SLineData       LineData[240];
extern __thread struct SLineMatrixData LineMatrixData[240];
...
bool8 S9xGraphicsInit(void)
{
  ...
  {
    uint32 r2 = r << 1;
    if (r2 > MAX_RED) r2 = MAX_RED;

    for (uint32 g = 0; g <= MAX_GREEN; g++)
    {
      uint32 g2 = g << 1;
      if (g2 > MAX_GREEN) g2 = MAX_GREEN;

      for (uint32 b = 0; b <= MAX_BLUE; b++)
      {
        uint32 b2 = b << 1;
        if (b2 > MAX_BLUE) b2 = MAX_BLUE;
...
void S9xStartScreenRefresh(void)
{
  ...
  if (GFX.InfoStringTimeout > 0 && --GFX.InfoStringTimeout == 0) GFX.InfoString = NULL;
...
void S9xEndScreenRefresh(void)
{
  if (IPPU.RenderThisFrame)
  {
    ...
    } else
    {
      ...
      if (Settings.TakeScreenshot) S9xDoScreenshot(IPPU.RenderedScreenWidth, IPPU.RenderedScreenHeight);

      if (Settings.AutoDisplayMessages) S9xDisplayMessages(GFX.Screen, GFX.RealPPL, IPPU.RenderedScreenWidth, IPPU.RenderedScreenHeight, 1);
...
#define DO_BG(n, pal, depth, hires, offset, Zh, Zl, voffoff)                                                                                                                       \
  ...
  case 6: DO_BG(0, 0, 4, TRUE, TRUE, 15, 7, 8); break;
...
{
  if (IPPU.OBJChanged || IPPU.InterlaceOBJ) SetupOBJ();
  ...
  if (!PPU.ForcedBlanking)
  {
    ...
    if (Settings.SupportHiRes)
    {
      if (!IPPU.DoubleWidthPixels && (PPU.BGMode == 5 || PPU.BGMode == 6 || IPPU.PseudoHires))
      {
        ...
        if (Settings.OpenGLEnable && GFX.RealPPL == 256)
        {
          ...
          for (int32 y = (int32)GFX.StartY - 1; y >= 0; y--)
          {
            ...
            for (int x = 255; x >= 0; x--, p--, q -= 2) *q = *(q + 1) = *p;
          }
      ...
      if (!IPPU.DoubleHeightPixels && IPPU.Interlace && (PPU.BGMode == 5 || PPU.BGMode == 6))
      {
        ...
        for (int32 y = (int32)GFX.StartY - 1; y >= 0; y--) memmove(GFX.Screen + y * GFX.PPL, GFX.Screen + y * GFX.RealPPL, IPPU.RenderedScreenWidth * sizeof(uint16));
      }
    }
  ...
  } else
  {
    ...
    for (uint32 l = GFX.StartY; l <= GFX.EndY; l++, GFX.S += GFX.PPL)
      for (int x = 0; x < IPPU.RenderedScreenWidth; x++) GFX.S[x] = black;
  }
...
static void SetupOBJ(void)
{
  ...
  if (!PPU.OAMPriorityRotation || !(PPU.OAMFlip & PPU.OAMAddr & 1)) // normal case
  {
    ...
    for (int Y = 1; Y < SNES_HEIGHT_EXTENDED; Y++) GFX.OBJLines[Y].RTOFlags |= GFX.OBJLines[Y - 1].RTOFlags;
  ...
  for (int clip = 0; clip < GFX.Clip[bg].Count; clip++)
  {
    ...
    for (uint32 Y = GFX.StartY; Y <= GFX.EndY; Y += Lines)
    {
      ...
      if (HPos & 7)
      {
        ...
        if (BG.TileSizeV == 16) Tile = TILE_PLUS(Tile, ((Tile & V_FLIP) ? t2 : t1));
...
void S9xReRefresh(void)
{
  ...
  if (Settings.Paused) S9xDeinitUpdate(IPPU.RenderedScreenWidth, IPPU.RenderedScreenHeight);
}
...
{
  if (linesFromBottom <= 0) linesFromBottom = 1;
  ...
  for (int i = 0; i < len; i++, char_count++)
  {
    if (char_count >= max_chars || (uint8)string[i] < 32)
    {
      ...
      if (dst >= GFX.Screen + IPPU.RenderedScreenHeight * GFX.RealPPL) break;

      char_count -= max_chars;
    }

    if ((uint8)string[i] < 32) continue;
...
static void DisplayFrameRate(void)
{
  ...
  {
    if (lastFrameCount < IPPU.TotalEmulatedFrames) { calcFps = (IPPU.TotalEmulatedFrames - lastFrameCount) / (uint32)(currTime - lastTime); }
    lastTime       = currTime;
...
static void DisplayPressedKeys(void)
{
  ...
  for (int port = 0; port < 2; port++)
  {
    ...
    case CTL_MOUSE:
    {
      ...
      uint8 buttons = buf[4];
      sprintf(string, "#%d %d: (%03d,%03d) %c%c", port, ids[0], x, y, (buttons & 0x40) ? 'L' : ' ', (buttons & 0x80) ? 'R' : ' ');
    ...
    case CTL_JUSTIFIER:
    {
      ...
      sprintf(string,
              "#%d %d: (%03d,%03d) %c%c%c / (%03d,%03d) %c%c%c",
              port,
              ids[0],
              x1,
              y1,
...
{
  if (Settings.DisplayFrameRate) DisplayFrameRate();

  if (Settings.DisplayWatchedAddresses) DisplayWatchedAddresses();

  if (Settings.DisplayPressedKeys) DisplayPressedKeys();

  if (Settings.DisplayMovieFrame && S9xMovieActive()) S9xDisplayString(GFX.FrameDisplayString, 1, 1, false);

  if (GFX.InfoString && *GFX.InfoString) S9xDisplayString(GFX.InfoString, 5, 1, true);
}
...
void S9xDrawCrosshair(const char *crosshair, uint8 fgcolor, uint8 bgcolor, int16 x, int16 y)
{
  ...
  for (r = 0; r < 15 * rx; r++, s += GFX.RealPPL - 15 * cx)
  {
    ...
    for (c = 0; c < 15 * cx; c++, s++)
    {
      if (x + c < 0 || s < GFX.Screen) continue;
...
static void DecomposePixelRGB5551(uint32, uint32 &, uint32 &, uint32 &);
  ...
  #define _BUILD_PIXEL(F)                                                                                                                                                          \
    static uint32 BuildPixel##F(uint32 R, uint32 G, uint32 B) { return (BUILD_PIXEL_##F(R, G, B)); }                                                                               \
                                                                                                                                                                                   \
    static uint32 BuildPixel2##F(uint32 R, uint32 G, uint32 B) { return (BUILD_PIXEL2_##F(R, G, B)); }                                                                             \
                                                                                                                                                                                   \
    static void DecomposePixel##F(uint32 pixel, uint32 &R, uint32 &G, uint32 &B) { DECOMPOSE_PIXEL_##F(pixel, R, G, B); }
...
bool8 S9xSetRenderPixelFormat(int format)
{
  ...
  switch (format)
  {
  case RGB565: _BUILD_SETUP(RGB565) return (TRUE);

  case RGB555: _BUILD_SETUP(RGB555) return (TRUE);

  case BGR565: _BUILD_SETUP(BGR565) return (TRUE);

  case BGR555: _BUILD_SETUP(BGR555) return (TRUE);

  case GBR565: _BUILD_SETUP(GBR565) return (TRUE);

  case GBR555: _BUILD_SETUP(GBR555) return (TRUE);

  case RGB5551: _BUILD_SETUP(RGB5551) return (TRUE);
