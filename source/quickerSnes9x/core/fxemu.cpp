...
void S9xInitSuperFX(void) { memset((uint8 *)&GSU, 0, sizeof(struct FxRegs_s)); }
...
void S9xSetSuperFX(uint8 byte, uint16 address)
{
  ...
  case 0x3031: Memory.FillRAM[0x3031] = byte; break;

  case 0x3033: Memory.FillRAM[0x3033] = byte; break;

  case 0x3034: Memory.FillRAM[0x3034] = byte & 0x7f; break;

  case 0x3036: Memory.FillRAM[0x3036] = byte & 0x7f; break;

  case 0x3037: Memory.FillRAM[0x3037] = byte; break;
  ...
  case 0x3039: Memory.FillRAM[0x3039] = byte; break;

  case 0x303a: Memory.FillRAM[0x303a] = byte; break;
  ...
  case 0x303f: Memory.FillRAM[0x303f] = byte; break;
  ...
  default:
    ...
    if (address >= 0x3100) FxCacheWriteAccess(address);
...
void S9xSuperFXExec(void)
{
  if ((Memory.FillRAM[0x3000 + GSU_SFR] & FLG_G) && (Memory.FillRAM[0x3000 + GSU_SCMR] & 0x18) == 0x18)
  {
    ...
    if ((GSUStatus & (FLG_G | FLG_IRQ)) == FLG_IRQ) CPU.IRQExternal = TRUE;
  }
}
...
static void FxReset(struct FxInfo_s *psFxInfo)
{
  ...
  if (GSU.nRomBanks > 0x20) GSU.nRomBanks = 0x20;
...
static void fx_readRegisterSpace(void)
{
  ...
  if (GSU.pvScreenBase + GSU.vScreenSize > GSU.pvRam + (GSU.nRamBanks * 65536)) GSU.pvScreenBase = GSU.pvRam + (GSU.nRamBanks * 65536) - GSU.vScreenSize;
...
static void fx_dirtySCBR(void) { GSU.vSCBRDirty = TRUE; }

static bool8 fx_checkStartAddress(void)
{
  // Check if we start inside the cache
  if (GSU.bCacheActive && R15 >= GSU.vCacheBaseReg && R15 < (GSU.vCacheBaseReg + 512)) return (TRUE);
  ...
  if (GSU.vPrgBankReg >= 0x60 && GSU.vPrgBankReg <= 0x6f) return (FALSE);

  if (GSU.vPrgBankReg >= 0x74) return (FALSE);

  // Check if we're in RAM and the RAN flag is not set
  if (GSU.vPrgBankReg >= 0x70 && GSU.vPrgBankReg <= 0x73 && !(SCMR & (1 << 3))) return (FALSE);

  // If not, we're in ROM, so check if the RON flag is set
  if (!(SCMR & (1 << 4))) return (FALSE);
...
static void FxCacheWriteAccess(uint16 vAddress)
{
  ...
  if ((vAddress & 0x00f) == 0x00f) GSU.vCacheFlags |= 1 << ((vAddress & 0x1f0) >> 4);
}
