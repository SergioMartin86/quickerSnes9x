...
static int16 DSP4_Inverse(int16 value)
{
  ...
  const uint16 div_lut[64] = {0x0000, 0x8000, 0x4000, 0x2aaa, 0x2000, 0x1999, 0x1555, 0x1249, 0x1000, 0x0e38, 0x0ccc, 0x0ba2, 0x0aaa, 0x09d8, 0x0924, 0x0888,
                              0x0800, 0x0787, 0x071c, 0x06bc, 0x0666, 0x0618, 0x05d1, 0x0590, 0x0555, 0x051e, 0x04ec, 0x04bd, 0x0492, 0x0469, 0x0444, 0x0421,
                              0x0400, 0x03e0, 0x03c3, 0x03a8, 0x038e, 0x0375, 0x035e, 0x0348, 0x0333, 0x031f, 0x030c, 0x02fa, 0x02e8, 0x02d8, 0x02c8, 0x02b9,
                              0x02aa, 0x029c, 0x028f, 0x0282, 0x0276, 0x026a, 0x025e, 0x0253, 0x0249, 0x023e, 0x0234, 0x022b, 0x0222, 0x0219, 0x0210, 0x0208};
...
static void DSP4_Multiply(int16 Multiplicand, int16 Multiplier, int32 *Product) { *Product = (Multiplicand * Multiplier << 1) >> 1; }
...
static void DSP4_OP01(void)
{
  ...
  do {
    ...
    if (DSP4.view_y2 < DSP4.poly_top[0][0])
    {
      ...
      if (DSP4.view_y1 >= DSP4.poly_top[0][0]) DSP4.segments = DSP4.view_y1 - DSP4.poly_top[0][0];
    }
...
static void DSP4_OP08(void)
{
  ...
  if (win_left < DSP4.poly_clipLf[0][0]) win_left = DSP4.poly_clipLf[0][0];
  if (win_left > DSP4.poly_clipRt[0][0]) win_left = DSP4.poly_clipRt[0][0];
  if (win_right < DSP4.poly_clipLf[0][1]) win_right = DSP4.poly_clipLf[0][1];
  if (win_right > DSP4.poly_clipRt[0][1]) win_right = DSP4.poly_clipRt[0][1];
  ...
  resume2:
    ...
    for (polygon = 0; polygon < 2; polygon++)
    {
      ...
      if (view_y[polygon] < DSP4.poly_top[polygon][0])
      {
        ...
        // flush remaining raster lines
        if (view_y[polygon] >= DSP4.poly_top[polygon][0]) DSP4.segments = view_y[polygon] - DSP4.poly_top[polygon][0];
      }
      ...
      if (DSP4.segments)
      {
        ...
        if (DSP4.segments == 1) left_inc = -left_inc;
        ...
        if (DSP4.segments == 1) right_inc = -right_inc;
        ...
        for (DSP4.lcv = 0; DSP4.lcv < DSP4.segments; DSP4.lcv++)
        {
          ...
          if (x_left < DSP4.poly_clipLf[polygon][0]) x_left = DSP4.poly_clipLf[polygon][0];
          if (x_left > DSP4.poly_clipRt[polygon][0]) x_left = DSP4.poly_clipRt[polygon][0];
          if (x_right < DSP4.poly_clipLf[polygon][1]) x_right = DSP4.poly_clipLf[polygon][1];
          if (x_right > DSP4.poly_clipRt[polygon][1]) x_right = DSP4.poly_clipRt[polygon][1];
...
static void DSP4_OP09(void)
{
  ...
  resume1:
    ...
    // op termination
    DSP4.distance = DSP4_READ_WORD();
    if (DSP4.distance == -0x8000) goto terminate;

    // no sprite
    if (DSP4.distance == 0x0000) continue;
    ...
    resume5:
      ...
      // opcode termination
      DSP4.raster = DSP4_READ_WORD();
      if (DSP4.raster == -0x8000) goto terminate;

      // stop code
      if (DSP4.raster == 0x0000 && !DSP4.sprite_size) break;
    ...
    resume6:
      ...
      if (DSP4.sprite_clipy - pixels <= sp_y && sp_y <= DSP4.sprite_clipy && sp_x >= DSP4.viewport_left - pixels && sp_x <= DSP4.viewport_right &&
          DSP4.sprite_clipy >= DSP4.viewport_top - pixels && DSP4.sprite_clipy <= DSP4.viewport_bottom)
...
{
  const uint16 OP0A_Values[16] = {0x0000, 0x0030, 0x0060, 0x0090, 0x00c0, 0x00f0, 0x0120, 0x0150, 0xfe80, 0xfeb0, 0xfee0, 0xff10, 0xff40, 0xff70, 0xffa0, 0xffd0};
...
static void DSP4_OP0B(bool8 *draw, int16 sp_x, int16 sp_y, int16 sp_attr, bool8 size, bool8 stop)
{
  ...
  // check boundaries
  if (!((sp_y < 0) || ((sp_y & 0x01ff) < 0x00eb))) *draw = 0;

  if (size)
  {
    if (DSP4.OAM_Row[Row1] + 1 >= DSP4.OAM_RowMax) *draw = 0;
    if (DSP4.OAM_Row[Row2] + 1 >= DSP4.OAM_RowMax) *draw = 0;
  } else
  {
    if (DSP4.OAM_Row[Row1] >= DSP4.OAM_RowMax) *draw = 0;
  }

  // emulator fail-safe (unknown if this really exists)
  if (DSP4.sprite_count >= 128) *draw = 0;
...
static void DSP4_SetByte(void)
{
  ...
  if (DSP4.waiting4command)
  {
    if (DSP4.half_command)
    {
      ...
      default: DSP4.waiting4command = TRUE; break;
      }
    } else
    {
  ...
  if (!DSP4.waiting4command && DSP4.in_count == DSP4.in_index)
  {
    ...
    // clear OAM
    case 0x0005: DSP4_OP05(); break;

    // transfer OAM
    case 0x0006: DSP4_OP06(); break;
    ...
    case 0x000F: DSP4_OP0F(); break;

    // single-player track turnoff projection with lighting
    case 0x0010: DSP4_OP10(); break;
...
static void DSP4_GetByte(void)
{
  if (DSP4.out_count)
  {
    ...
    DSP4.out_index++;
    if (DSP4.out_count == DSP4.out_index) DSP4.out_count = 0;
  } else
    DSP4.byte = 0xff;
}
