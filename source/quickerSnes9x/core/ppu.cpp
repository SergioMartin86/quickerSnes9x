...
extern __thread uint8 *HDMAMemPointers[8];
...
static inline void S9xLatchCounters(bool force)
{
  if (force || (Memory.FillRAM[0x4213] & 0x80))
  {
    ...
    {
      if (hc >= 1292) hc -= (ONE_DOT_CYCLE / 2);
      if (hc >= 1308) hc -= (ONE_DOT_CYCLE / 2);
    }
  ...
  if (CPU.V_Counter > PPU.GunVLatch || (CPU.V_Counter == PPU.GunVLatch && CPU.Cycles >= PPU.GunHLatch * ONE_DOT_CYCLE)) PPU.GunVLatch = 1000;
}
...
void S9xUpdateHVTimerPosition(void)
{
  ...
  if (Timings.H_Max == Timings.H_Max_Master) // 1364
  {
    if (PPU.IRQHBeamPos > 322) PPU.HTimerPosition += (ONE_DOT_CYCLE / 2);
    if (PPU.IRQHBeamPos > 326) PPU.HTimerPosition += (ONE_DOT_CYCLE / 2);
  }
  ...
  if ((PPU.HTimerPosition >= Timings.H_Max) && (PPU.IRQHBeamPos < 340))
  {
    ...
    PPU.VTimerPosition++;
    // FIXME
    if (PPU.VTimerPosition >= Timings.V_Max) PPU.VTimerPosition = 0;
  }

#ifdef DEBUGGER
  S9xTraceFormattedMessage("--- IRQ Timer set  HTimer:%d Pos:%04d  VTimer:%d Pos:%03d", PPU.HTimerEnabled, PPU.HTimerPosition, PPU.VTimerEnabled, PPU.VTimerPosition);
#endif
}
...
void S9xSetPPU(uint8 Byte, uint16 Address)
{
  ...
  if (CPU.InDMAorHDMA)
  {
    ...
    } else
    {
      ...
      if (Address > 0x21ff) Address = 0x2100 + (Address & 0xff);
    }
  }

#ifdef DEBUGGER
  if (CPU.InHDMA) S9xTraceFormattedMessage("--- HDMA PPU %04X -> %02X", Address, Byte);
#endif

  if (Settings.MSU1 && (Address & 0xfff8) == 0x2000) // MSU-1
    S9xMSU1WritePort(Address & 7, Byte);
  else if ((Address & 0xffc0) == 0x2140) // APUIO0, APUIO1, APUIO2, APUIO3
  ...
  else if (Address <= 0x2183)
  {
    ...
    case 0x2100: // INIDISP
      ...
      if ((Memory.FillRAM[0x2100] & 0x80) && CPU.V_Counter == PPU.ScreenHeight + FIRST_VISIBLE_LINE)
      {
        ...
        uint8 tmp = 0;
        if (PPU.OAMPriorityRotation) tmp = (PPU.OAMAddr & 0xfe) >> 1;
...
#ifdef DEBUGGER
        if ((Byte & 0xf0) && (Byte & 0x0f)) missing.mosaic = 1;
#endif
      }
    ...
    case 0x2115: // VMAIN
      ...
      if (Byte & 0x0c)
      {
        static __thread uint16 Shift[4]    = {0, 5, 6, 7};
        static __thread uint16 IncCount[4] = {0, 32, 64, 128};
...
#ifdef DEBUGGER
      if (Byte & 3) missing.vram_inc = Byte & 3;
#endif
      break;
...
#ifdef DEBUGGER
        if (Byte & 0x80) missing.window2[1] = 1;
        if (Byte & 0x20) missing.window1[1] = 1;
        if (Byte & 0x08) missing.window2[0] = 1;
        if (Byte & 0x02) missing.window1[0] = 1;
#endif
      }
    ...
    case 0x2124: // W34SEL
      if (Byte != Memory.FillRAM[0x2124])
      {
        ...
        PPU.RecomputeClipWindows = TRUE;
#ifdef DEBUGGER
        if (Byte & 0x80) missing.window2[3] = 1;
        if (Byte & 0x20) missing.window1[3] = 1;
        if (Byte & 0x08) missing.window2[2] = 1;
        if (Byte & 0x02) missing.window1[2] = 1;
#endif
      }
    ...
    case 0x2125: // WOBJSEL
      if (Byte != Memory.FillRAM[0x2125])
      {
        ...
        PPU.RecomputeClipWindows = TRUE;
#ifdef DEBUGGER
        if (Byte & 0x80) missing.window2[5] = 1;
        if (Byte & 0x20) missing.window1[5] = 1;
        if (Byte & 0x08) missing.window2[4] = 1;
        if (Byte & 0x02) missing.window1[4] = 1;
#endif
      }
    ...
    case 0x212d: // TS
      if (Byte != Memory.FillRAM[0x212d])
      {
        ...
        PPU.RecomputeClipWindows = TRUE;
#ifdef DEBUGGER
        if (Byte & 0x1f) missing.subscreen = 1;
#endif
      }
    ...
    case 0x2130: // CGWSEL
      if (Byte != Memory.FillRAM[0x2130])
      {
        ...
        PPU.RecomputeClipWindows = TRUE;
#ifdef DEBUGGER
        if ((Byte & 1) && (PPU.BGMode == 3 || PPU.BGMode == 4 || PPU.BGMode == 7)) missing.direct = 1;
#endif
      }
    ...
    case 0x2132: // COLDATA
      ...
      {
        FLUSH_REDRAW();
        if (Byte & 0x80) PPU.FixedColourBlue = Byte & 0x1f;
        if (Byte & 0x40) PPU.FixedColourGreen = Byte & 0x1f;
        if (Byte & 0x20) PPU.FixedColourRed = Byte & 0x1f;
      }
    ...
    case 0x2133: // SETINI
      if (Byte != Memory.FillRAM[0x2133])
      {
        ...
        {
          FLUSH_REDRAW();
          if ((Memory.FillRAM[0x2133] ^ Byte) & 2) IPPU.OBJChanged = TRUE;
          IPPU.Interlace    = Byte & 1;
          IPPU.InterlaceOBJ = Byte & 2;
        }
#ifdef DEBUGGER
        if (Byte & 0x40) missing.mode7_bgmode = 1;
        if (Byte & 0x08) missing.pseudo_512 = 1;
        if (Byte & 0x02) missing.sprite_double_height = 1;
        if (Byte & 0x01) missing.interlace = 1;
#endif
      }
    ...
    case 0x2180: // WMDATA
      if (!CPU.InWRAMDMAorHDMA) REGISTER_2180(Byte);
      break;
...
uint8 S9xGetPPU(uint16 Address)
{
  ...
  } else
  {
    ...
    else if (Settings.SRTC && Address == 0x2800)
      return (S9xGetSRTC(Address));
    else
      switch (Address)
      {
      case 0x21c2:
        if (Model->_5C77 == 2) return (0x20);
        return (OpenBus);
...
void S9xSetCPU(uint8 Byte, uint16 Address)
{
  ...
  } else
  {
    ...
    case 0x4200: // NMITIMEN
      ...
      if (CPU.IRQLine && !PPU.HTimerEnabled && PPU.VTimerEnabled) CPU.IRQTransition = TRUE;
...
#ifdef DEBUGGER
      S9xTraceFormattedMessage("--- IRQ Timer Enable HTimer:%d Pos:%04d  VTimer:%d Pos:%03d", PPU.HTimerEnabled, PPU.HTimerPosition, PPU.VTimerEnabled, PPU.VTimerPosition);
#endif
    ...
    case 0x4207: // HTIMEL
      ...
      if (PPU.IRQHBeamPos != pos) S9xUpdateHVTimerPosition();
#ifdef DEBUGGER
      missing.hirq_pos = PPU.IRQHBeamPos;
#endif
      break;
    ...
    case 0x4209: // VTIMEL
      ...
      if (PPU.IRQVBeamPos != pos) S9xUpdateHVTimerPosition();
#ifdef DEBUGGER
      missing.virq_pos = PPU.IRQVBeamPos;
#endif
      break;
    ...
    case 0x420b: // MDMAEN
      ...
      if (Byte & 0x01) S9xDoDMA(0);
      if (Byte & 0x02) S9xDoDMA(1);
      if (Byte & 0x04) S9xDoDMA(2);
      if (Byte & 0x08) S9xDoDMA(3);
      if (Byte & 0x10) S9xDoDMA(4);
      if (Byte & 0x20) S9xDoDMA(5);
      if (Byte & 0x40) S9xDoDMA(6);
      if (Byte & 0x80) S9xDoDMA(7);
#ifdef DEBUGGER
...
uint8 S9xGetCPU(uint16 Address)
{
  ...
  } else
  {
    ...
    default:
      if (Settings.SPC7110 && Address >= 0x4800) return (S9xGetSPC7110(Address));
      if (Settings.SDD1 && Address >= 0x4800 && Address <= 0x4807) return (Memory.FillRAM[Address]);
      return (OpenBus);
    }
  }
}
...
void S9xSoftResetPPU(void)
{
  ...
  for (int c = 0; c < 2; c++) memset(&IPPU.Clip[c], 0, sizeof(struct ClipData));
  IPPU.ColorsChanged               = TRUE;
  IPPU.OBJChanged                  = TRUE;
  IPPU.DirectColourMapsNeedRebuild = TRUE;
  ...
  IPPU.PreviousLine       = 0;
  IPPU.XB                 = NULL;
  for (int c = 0; c < 256; c++) IPPU.ScreenColors[c] = c;
  ...
  for (int c = 0; c < 0x8000; c += 0x100) memset(&Memory.FillRAM[c], c >> 8, 0x100);
