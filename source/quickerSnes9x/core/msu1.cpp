...
bool AudioOpen()
{
  ...
  if (audioFile.good())
  {
    if (audioFile.get() != 'M') return false;
    if (audioFile.get() != 'S') return false;
    if (audioFile.get() != 'U') return false;
    if (audioFile.get() != '1') return false;
...
uint8 S9xMSU1ReadPort(int port)
{
  switch (port)
  {
  case 0: return MSU1.MSU1_STATUS;
  case 1:
    if (MSU1.MSU1_STATUS & DataBusy) return 0;
...
void S9xMSU1WritePort(int port, uint8 byte)
{
  ...
  case 3:
    ...
    if (dataFile.good()) dataFile.seekg(MSU1.MSU1_DATA_POS);
    break;
  case 4:
  ...
  case 5:
    ...
    break;
  case 6: MSU1.MSU1_VOLUME = byte; break;
  case 7:
    if (MSU1.MSU1_STATUS & (AudioBusy | AudioError)) break;
...
uint16 S9xMSU1Samples(void) { return bufPos - bufBegin; }
...
{
  if (DataOpen()) { dataFile.seekg(MSU1.MSU1_DATA_POS); }
