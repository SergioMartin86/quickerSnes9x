...
static void S9xSA1UpdateTimer(void) // FIXME
{
  ...
  if (Memory.FillRAM[0x2210] & 0x80)
  {
    ...
    if (SA1.HCounter >= 0x800)
    {
      ...
      if (++SA1.VCounter >= 0x200) SA1.VCounter = 0;
    }
  } else
  {
    SA1.HCounter += (SA1.Cycles - SA1.PrevCycles);
    if (SA1.HCounter >= Timings.H_Max_Master)
    {
      ...
      if (++SA1.VCounter >= Timings.V_Max_Master) SA1.VCounter = 0;
    }
  }

  if (SA1.Cycles >= Timings.H_Max_Master) SA1.Cycles -= Timings.H_Max_Master;
  ...
  {
    if (SA1.PrevHCounter >= SA1.HTimerIRQPos * ONE_DOT_CYCLE || SA1.HCounter < SA1.HTimerIRQPos * ONE_DOT_CYCLE) thisIRQ = FALSE;
  }

  if (Memory.FillRAM[0x2210] & 0x02)
  {
    if (SA1.VCounter != SA1.VTimerIRQPos * ONE_DOT_CYCLE) thisIRQ = FALSE;
  }
  ...
  if (!SA1.TimerIRQLastState && thisIRQ)
  {
    ...
    if (Memory.FillRAM[0x220a] & 0x40)
    {
      ...
      S9xTraceFormattedMessage("--- SA-1 Timer IRQ triggered  prev HC:%04d  curr HC:%04d  HTimer:%d Pos:%04d  VTimer:%d Pos:%03d",
                               SA1.PrevHCounter,
                               SA1.HCounter,
                               (Memory.FillRAM[0x2210] & 0x01) ? 1 : 0,
                               SA1.HTimerIRQPos * ONE_DOT_CYCLE,
                               (Memory.FillRAM[0x2210] & 0x02) ? 1 : 0,
                               SA1.VTimerIRQPos);
#endif
    }
  }
