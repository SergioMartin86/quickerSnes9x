...
ConfigFile::ConfigFile(void) { Clear(); }
...
void ConfigFile::LoadFile(Stream *r, const char *name)
{
  ...
  do {
    ...
    key = l.substr(0, i);
    ConfigEntry::trim(key);
    val     = l.substr(i + 1);
    comment = ConfigEntry::trimCommented(val);
    if (val.size() > 0 && val[0] == '"' && *val.rbegin() == '"') val = val.substr(1, val.size() - 2);

    ConfigEntry e(line, section, key, val);
    e.comment = comment;
    if (data.erase(e)) sectionSizes.DecreaseSectionSize(e.section);
    data.insert(e);
...
bool ConfigFile::SaveTo(const char *filename) { return true; }
...
bool ConfigFile::Has(const char *key) { return data.find(ConfigEntry(key)) != data.end(); }
...
bool ConfigFile::SetString(const char *key, string val, const char *comment)
{
  ...
  if ((found && (!alphaSort || timeSort)) || (!alphaSort && timeSort)) e.line = linectr++;
...
bool ConfigFile::SetUInt(const char *key, uint32 val, int base, const char *comment)
{
  ...
  {
  case 10:
  default: snprintf(buf, sizeof(buf), "%u", (unsigned int)val); break;
  case 8: snprintf(buf, sizeof(buf), "%#o", (unsigned int)val); break;
  case 16: snprintf(buf, sizeof(buf), "%#x", (unsigned int)val); break;
  }
  return SetString(key, buf, comment);
}
...
int ConfigFile::GetSectionSize(const std::string section) { return sectionSizes.GetSectionSize(section); }
...
{
  set<ConfigEntry, ConfigEntry::key_less>::iterator i;
  for (i = data.begin(); i != data.end(); i++) { *(const_cast<int *>(&i->line)) = -1; }
}
...
void ConfigFile::SetDefaultAutoAdd(bool autoAdd) { defaultAutoAdd = autoAdd; }
void ConfigFile::SetNiceAlignment(bool align) { niceAlignment = align; }
void ConfigFile::SetShowComments(bool show) { showComments = show; }
void ConfigFile::SetAlphaSort(bool sort) { alphaSort = sort; }
void ConfigFile::SetTimeSort(bool sort) { timeSort = sort; }
