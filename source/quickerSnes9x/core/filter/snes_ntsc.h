...
void snes_ntsc_blit(snes_ntsc_t const *ntsc, SNES_NTSC_IN_T const *input, long in_row_width, int burst_phase, int in_width, int in_height, void *rgb_out, long out_pitch);

void snes_ntsc_blit_hires(snes_ntsc_t const *ntsc, SNES_NTSC_IN_T const *input, long in_row_width, int burst_phase, int in_width, int in_height, void *rgb_out, long out_pitch);
...
#define SNES_NTSC_BEGIN_ROW(ntsc, burst, pixel0, pixel1, pixel2)                                                                                                                   \
  char const *ktable = (char const *)(ntsc)->table + burst * (snes_ntsc_burst_size * sizeof(snes_ntsc_rgb_t));                                                                     \
...
#define SNES_NTSC_HIRES_OUT(x, rgb_out, bits)                                                                                                                                      \
  {                                                                                                                                                                                \
    snes_ntsc_rgb_t raw_ = kernel0[x] + kernel2[(x + 5) % 7 + 14] + kernel4[(x + 3) % 7 + 28] + kernelx0[(x + 7) % 7 + 7] + kernelx2[(x + 5) % 7 + 21] +                           \
                           kernelx4[(x + 3) % 7 + 35] + kernel1[(x + 6) % 7] + kernel3[(x + 4) % 7 + 14] + kernel5[(x + 2) % 7 + 28] + kernelx1[(x + 6) % 7 + 7] +                 \
                           kernelx3[(x + 4) % 7 + 21] + kernelx5[(x + 2) % 7 + 35];                                                                                                \
    SNES_NTSC_CLAMP_(raw_, 0);                                                                                                                                                     \
    SNES_NTSC_RGB_OUT_(rgb_out, (bits), 0);                                                                                                                                        \
  }

/* private */
const int snes_ntsc_entry_size = 128;
enum
{
  snes_ntsc_palette_size = 0x2000
};
...
#define SNES_NTSC_RGB16(ktable, n) (snes_ntsc_rgb_t const *)(ktable + ((n & 0x001E) | (n >> 1 & 0x03E0) | (n >> 2 & 0x3C00)) * (snes_ntsc_entry_size / 2 * sizeof(snes_ntsc_rgb_t)))

#define SNES_NTSC_RGB15(ktable, n) (snes_ntsc_rgb_t const *)(ktable + ((n & 0x001E) | (n >> 0 & 0x03E0) | (n >> 1 & 0x3C00)) * (snes_ntsc_entry_size / 2 * sizeof(snes_ntsc_rgb_t)))

#define SNES_NTSC_BGR15(ktable, n)                                                                                                                                                 \
  (snes_ntsc_rgb_t const *)(ktable + ((n << 9 & 0x3C00) | (n & 0x03E0) | (n >> 10 & 0x001E)) * (snes_ntsc_entry_size / 2 * sizeof(snes_ntsc_rgb_t)))
...
#define SNES_NTSC_RGB_OUT_14_(x, rgb_out, bits, shift)                                                                                                                             \
  {                                                                                                                                                                                \
    snes_ntsc_rgb_t raw_ =                                                                                                                                                         \
      kernel0[x] + kernel1[(x + 12) % 7 + 14] + kernel2[(x + 10) % 7 + 28] + kernelx0[(x + 7) % 14] + kernelx1[(x + 5) % 7 + 21] + kernelx2[(x + 3) % 7 + 35];                     \
...
#define SNES_NTSC_RGB_OUT_(rgb_out, bits, x)                                                                                                                                       \
  {                                                                                                                                                                                \
    if (bits == 16) rgb_out = (raw_ >> (13 - x) & 0xF800) | (raw_ >> (8 - x) & 0x07E0) | (raw_ >> (4 - x) & 0x001F);                                                               \
    if (bits == 24 || bits == 32) rgb_out = (raw_ >> (5 - x) & 0xFF0000) | (raw_ >> (3 - x) & 0xFF00) | (raw_ >> (1 - x) & 0xFF);                                                  \
    if (bits == 15) rgb_out = (raw_ >> (14 - x) & 0x7C00) | (raw_ >> (9 - x) & 0x03E0) | (raw_ >> (4 - x) & 0x001F);                                                               \
    if (bits == 14) rgb_out = (raw_ >> (24 - x) & 0x001F) | (raw_ >> (9 - x) & 0x03E0) | (raw_ << (6 + x) & 0x7C00);                                                               \
    if (bits == 0) rgb_out = raw_ << x;                                                                                                                                            \
  }
