...
static void merge_kernel_fields(snes_ntsc_rgb_t *io)
{
  ...
  for (n = burst_size; n; --n)
  {
    ...
    io[burst_size * 0] = ((p0 + p1 - ((p0 ^ p1) & snes_ntsc_rgb_builder)) >> 1) - rgb_bias;
    io[burst_size * 1] = ((p1 + p2 - ((p1 ^ p2) & snes_ntsc_rgb_builder)) >> 1) - rgb_bias;
    io[burst_size * 2] = ((p2 + p0 - ((p2 ^ p0) & snes_ntsc_rgb_builder)) >> 1) - rgb_bias;
    ++io;
  }
}
...
static void correct_errors(snes_ntsc_rgb_t color, snes_ntsc_rgb_t *out)
{
  ...
  for (n = burst_count; n; --n)
  {
    ...
    {
      snes_ntsc_rgb_t error = color - out[i] - out[(i + 12) % 14 + 14] - out[(i + 10) % 14 + 28] - out[i + 7] - out[i + 5 + 14] - out[i + 3 + 28];
...
void snes_ntsc_init(snes_ntsc_t *ntsc, snes_ntsc_setup_t const *setup)
{
  ...
  int    entry;
  init_t impl;
  if (!setup) setup = &snes_ntsc_composite;
  init(&impl, setup);

  merge_fields = setup->merge_fields;
  if (setup->artifacts <= -1 && setup->fringing <= -1) merge_fields = 1;
...
void snes_ntsc_blit(snes_ntsc_t const *ntsc, SNES_NTSC_IN_T const *input, long in_row_width, int burst_phase, int in_width, int in_height, void *rgb_out, long out_pitch)
{
  ...
  for (; in_height; --in_height)
  {
    ...
    SNES_NTSC_BEGIN_ROW(ntsc, burst_phase, snes_ntsc_black, snes_ntsc_black, SNES_NTSC_ADJ_IN(*line_in));
...
void snes_ntsc_blit_hires(snes_ntsc_t const *ntsc, SNES_NTSC_IN_T const *input, long in_row_width, int burst_phase, int in_width, int in_height, void *rgb_out, long out_pitch)
{
  ...
  {
    SNES_NTSC_IN_T const *line_in = input;
    SNES_NTSC_HIRES_ROW(ntsc, burst_phase, snes_ntsc_black, snes_ntsc_black, snes_ntsc_black, SNES_NTSC_ADJ_IN(line_in[0]), SNES_NTSC_ADJ_IN(line_in[1]));
    snes_ntsc_out_t *restrict line_out = (snes_ntsc_out_t *)rgb_out;
    int n;
    line_in += 2;
