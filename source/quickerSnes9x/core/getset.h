...
extern __thread uint8 OpenBus;

static inline int32 memory_speed(uint32 address)
{
  if (address & 0x408000)
  {
    if (address & 0x800000) return (CPU.FastROMSpeed);

    return (SLOW_ONE_CYCLE);
  }

  if ((address + 0x6000) & 0x4000) return (SLOW_ONE_CYCLE);

  if ((address - 0x4000) & 0x7e00) return (ONE_CYCLE);
...
inline uint16 S9xGetWord(uint32 Address, enum s9xwrap_t w = WRAP_NONE)
{
  ...
  if ((Address & mask) == mask)
  {
    ...
    case WRAP_NONE:
    default: return (OpenBus | (S9xGetByte(Address + 1) << 8));
    }
  }
...
inline void S9xSetByte(uint8 Byte, uint32 Address)
{
  ...
  case CMemory::MAP_NONE:
  default: addCyclesInMemoryAccess; return;
  }
}
...
inline void S9xSetWord(uint16 Word, uint32 Address, enum s9xwrap_t w = WRAP_NONE, enum s9xwriteorder_t o = WRITE_01)
{
  ...
  if ((Address & mask) == mask)
  {
    ...
    if (!o) S9xSetByte((uint8)Word, Address);
    ...
    case WRAP_NONE:
    default: S9xSetByte(Word >> 8, Address + 1); break;
    }
  ...
  case CMemory::MAP_NONE:
  default: addCyclesInMemoryAccess_x2; return;
  }
}
...
inline void S9xSetPCBase(uint32 Address)
{
  ...
  case CMemory::MAP_BWRAM: CPU.PCBase = Memory.BWRAM - 0x6000 - (Address & 0x8000); return;

  case CMemory::MAP_SA1RAM: CPU.PCBase = Memory.SRAM; return;

  case CMemory::MAP_SPC7110_ROM: CPU.PCBase = S9xGetBasePointerSPC7110(Address); return;

  case CMemory::MAP_C4: CPU.PCBase = S9xGetBasePointerC4(Address & 0xffff); return;

  case CMemory::MAP_OBC_RAM: CPU.PCBase = S9xGetBasePointerOBC1(Address & 0xffff); return;

  case CMemory::MAP_BSX: CPU.PCBase = S9xGetBasePointerBSX(Address); return;

  case CMemory::MAP_NONE:
  default: CPU.PCBase = NULL; return;
  }
}
...
inline uint8 *S9xGetBasePointer(uint32 Address)
{
  ...
  if (GetAddress >= (uint8 *)CMemory::MAP_LAST) return (GetAddress);
  ...
  case CMemory::MAP_BWRAM: return (Memory.BWRAM - 0x6000 - (Address & 0x8000));

  case CMemory::MAP_SA1RAM: return (Memory.SRAM);

  case CMemory::MAP_SPC7110_ROM: return (S9xGetBasePointerSPC7110(Address));

  case CMemory::MAP_C4: return (S9xGetBasePointerC4(Address & 0xffff));

  case CMemory::MAP_OBC_RAM: return (S9xGetBasePointerOBC1(Address & 0xffff));
...
inline uint8 *S9xGetMemPointer(uint32 Address)
{
  ...
  case CMemory::MAP_BWRAM: return (Memory.BWRAM - 0x6000 + (Address & 0x7fff));

  case CMemory::MAP_SA1RAM: return (Memory.SRAM + (Address & 0xffff));

  case CMemory::MAP_SPC7110_ROM: return (S9xGetBasePointerSPC7110(Address) + (Address & 0xffff));

  case CMemory::MAP_C4: return (S9xGetMemPointerC4(Address & 0xffff));

  case CMemory::MAP_OBC_RAM: return (S9xGetMemPointerOBC1(Address & 0xffff));
