...
void S9xNPShutdownClient(int c, bool8 report_error = FALSE)
{
  if (NPServer.Clients[c].Connected)
  {
    ...
    if (report_error)
    {
      sprintf(NetPlay.ErrorMsg, "Player %d on '%s' has disconnected.", c + 1, NPServer.Clients[c].HostName);
...
static bool8 S9xNPSGetData(int socket, uint8 *data, int length)
{
  ...
  do {
    ...
    if (num_bytes > 512) num_bytes = 512;
...
static bool8 S9xNPSSendData(int fd, const uint8 *data, int length)
{
  ...
  do {
    ...
    // abort request from another thread.
    if (num_bytes > chunk) num_bytes = chunk;
...
void S9xNPSendHeartBeat()
{
  ...
  {
    if (NPServer.Clients[n].SaidHello) break;
  }
...
void S9xNPSendToAllClients(uint8 *data, int len)
{
  ...
  for (i = 0; i < NP_MAX_CLIENTS; i++)
  {
    if (NPServer.Clients[i].SaidHello)
    {
      ...
      if (!S9xNPSSendData(NPServer.Clients[i].Socket, data, len)) S9xNPShutdownClient(i, TRUE);
    }
  }
}
...
void S9xNPProcessClient(int c)
{
  ...
  case NP_CLNT_READY:
    ...
    break;
  case NP_CLNT_JOYPAD: NPServer.Joypads[c] = len; break;
  case NP_CLNT_PAUSE:
  #ifdef NP_DEBUG
...
void S9xNPAcceptClient(int Listen, bool8 block)
{
  ...
  if (remote_address.sin_family == AF_INET)
  {
    ...
    } else
    {
      ...
      if (ip) NPServer.Clients[i].HostName = strdup(ip);
  #ifdef NP_DEBUG
...
static bool8 S9xNPServerInit(int port)
{
  ...
  if (!S9xNPInitialise()) return (FALSE);
...
void S9xNPServerLoop(void *)
{
  ...
  while (server_continue)
  {
    ...
    if (success && !(Settings.Paused && !Settings.FrameAdvance) && !Settings.StopEmulation && !Settings.ForcedPause && !NPServer.Paused)
    {
      S9xNPSendHeartBeat();
      newPausedState = 0;
    } else
    ...
    do {
      ...
      for (i = 0; i < NP_MAX_CLIENTS; i++)
      {
        if (NPServer.Clients[i].Connected)
        {
          ...
          if (NPServer.Clients[i].Socket > max_fd) max_fd = NPServer.Clients[i].Socket;
        }
      }
      ...
      if (res > 0)
      {
        if (FD_ISSET(NPServer.Socket, &read_fds)) S9xNPAcceptClient(NPServer.Socket, FALSE);

        for (i = 0; i < NP_MAX_CLIENTS; i++)
        {
          if (NPServer.Clients[i].Connected && FD_ISSET(NPServer.Clients[i].Socket, &read_fds)) { S9xNPProcessClient(i); }
        }
      }
    }
    while (res > 0);
    ...
    while (NPServer.TaskHead != NPServer.TaskTail)
    {
      ...
      {
      case NP_SERVER_SEND_ROM_IMAGE: S9xNPSendROMImageToAllClients(); break;
      case NP_SERVER_SYNC_CLIENT:
        NPServer.Clients[(pint)task_data].Ready = FALSE;
        S9xNPRecomputePause();
        S9xNPSyncClient((pint)task_data);
        break;
      case NP_SERVER_SYNC_ALL: S9xNPSyncClients(); break;
      ...
      default: S9xNPSetError("SERVER: *** Unknown task ***\n"); break;
      }
...
void S9xNPStopServer()
{
  ...
  {
    if (NPServer.Clients[i].Connected) S9xNPShutdownClient(i, FALSE);
  }
}
...
void S9xNPSendROMImageToAllClients()
{
  ...
  {
    if (NPServer.Clients[c].SaidHello) S9xNPSendROMImageToClient(c);
  }
...
bool8 S9xNPSendROMImageToClient(int c)
{
  ...
  *ptr++ = Memory.HiROM;
  ...
  if (!S9xNPSSendData(NPServer.Clients[c].Socket, header, sizeof(header)) || !S9xNPSSendData(NPServer.Clients[c].Socket, Memory.ROM, Memory.CalculatedSize) ||
...
void S9xNPSendFreezeFile(int c, uint8 *data, uint32 len)
{
  ...
  *ptr++ = NP_SERV_FREEZE_FILE;
  ...
  if (!S9xNPSSendData(NPServer.Clients[c].Socket, header, 7 + 4) || !S9xNPSSendData(NPServer.Clients[c].Socket, data, len)) { S9xNPShutdownClient(c, TRUE); }
  S9xNPSetAction("", TRUE);
}
...
void S9xNPNoClientReady(int start_index)
{
  ...
  for (c = start_index; c < NP_MAX_CLIENTS; c++) NPServer.Clients[c].Ready = FALSE;
  S9xNPRecomputePause();
}
...
void S9xNPSendSRAMToAllClients()
{
  ...
  {
    if (NPServer.Clients[i].SaidHello) S9xNPSendSRAMToClient(i);
  }
}
...
void S9xNPSendSRAMToClient(int c)
{
  ...
  if (SRAMSize > 0x10000) SRAMSize = 0x10000;
  int len = 7 + SRAMSize;
  ...
  *ptr++     = NP_SERV_SRAM_DATA;
  WRITE_LONG(ptr, len);
  if (!S9xNPSSendData(NPServer.Clients[c].Socket, sram, sizeof(sram)) || (len > 7 && !S9xNPSSendData(NPServer.Clients[c].Socket, Memory.SRAM, len - 7)))
  {
    S9xNPShutdownClient(c, TRUE);
  }
}
...
void S9xNPSendFreezeFileToAllClients(const char *filename)
{
  ...
  if (NPServer.NumClients > NP_ONE_CLIENT && S9xNPLoadFreezeFile(filename, data, len))
  {
    ...
    {
      if (NPServer.Clients[c].SaidHello) S9xNPSendFreezeFile(c, data, len);
    }
    delete data;
  }
}
...
void S9xNPWaitForEmulationToComplete()
{
  ...
  while (!NetPlay.PendingWait4Sync && NetPlay.Connected && !Settings.ForcedPause && !Settings.StopEmulation && !(Settings.Paused && !Settings.FrameAdvance))
  {
  #ifdef __WIN32__
    Sleep(40);
  #endif
