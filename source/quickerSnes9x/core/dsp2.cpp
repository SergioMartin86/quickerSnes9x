...
static void DSP2_Op03(void) { DSP2.Op05Transparent = DSP2.parameters[0]; }
...
static void DSP2_Op06(void)
{
  ...
  for (int32 i = 0, j = DSP2.Op06Len - 1; i < DSP2.Op06Len; i++, j--) DSP2.output[j] = (DSP2.parameters[i] << 4) | (DSP2.parameters[i] >> 4);
}
...
static void DSP2_Op0D(void)
{
  ...
  for (int32 i = 0; i < DSP2.Op0DOutLen; i++) DSP2.output[i] = (pixelarray[i << 1] << 4) | pixelarray[(i << 1) + 1];
}
    ...
    if (DSP2.in_count == DSP2.in_index)
    {
      ...
      case 0x05:
        ...
        } else
        {
          ...
          if (byte) DSP2.waiting4command = FALSE;
        }
...
uint8 DSP2GetByte(uint16 address)
{
  ...
  if ((address & 0xf000) == 0x6000 || (address >= 0x8000 && address < 0xc000))
  {
    if (DSP2.out_count)
    {
      ...
      DSP2.out_index++;
      if (DSP2.out_count == DSP2.out_index) DSP2.out_count = 0;
    } else
      t = 0xff;
  } else
    t = 0x80;
