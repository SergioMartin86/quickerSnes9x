...
static inline void FLUSH_REDRAW(void)
{
  if (doRendering)
  {
    if (IPPU.PreviousLine != IPPU.CurrentLine) S9xUpdateScreen();
  }
}
...
// This code is correct, however due to Snes9x's inaccurate timings, some games might be broken by this chage. :(
  ...
  #define CHECK_INBLANK()                                                                                                                                                          \
    ...
    {                                                                                                                                                                              \
      ...
      if (Settings.BlockInvalidVRAMAccess) return;                                                                                                                                 \
    }
#else
  #define CHECK_INBLANK()                                                                                                                                                          \
    if (Settings.BlockInvalidVRAMAccess && !PPU.ForcedBlanking && CPU.V_Counter < PPU.ScreenHeight + FIRST_VISIBLE_LINE) return;
#endif
...
static inline void REGISTER_2118(uint8 Byte)
{
  ...
  {
#ifdef DEBUGGER
    if (Settings.TraceVRAM && !CPU.InDMAorHDMA) printf("VRAM write byte: $%04X (%d, %d)\n", PPU.VMA.Address, Memory.FillRAM[0x2115] & 3, (Memory.FillRAM[0x2115] & 0x0c) >> 2);
#endif
...
static inline void REGISTER_2119(uint8 Byte)
{
  ...
  {
#ifdef DEBUGGER
    if (Settings.TraceVRAM && !CPU.InDMAorHDMA) printf("VRAM write word: $%04X (%d, %d)\n", PPU.VMA.Address, Memory.FillRAM[0x2115] & 3, (Memory.FillRAM[0x2115] & 0x0c) >> 2);
#endif
    PPU.VMA.Address += PPU.VMA.Increment;
  }
}
...
static inline void REGISTER_2118_tile(uint8 Byte)
{
  ...
  if (!PPU.VMA.High) PPU.VMA.Address += PPU.VMA.Increment;
}
...
static inline uint8 REGISTER_4212(void)
{
  ...
  if ((CPU.V_Counter >= PPU.ScreenHeight + FIRST_VISIBLE_LINE) && (CPU.V_Counter < PPU.ScreenHeight + FIRST_VISIBLE_LINE + 3)) byte = 1;
  if ((CPU.Cycles < Timings.HBlankEnd) || (CPU.Cycles >= Timings.HBlankStart)) byte |= 0x40;
  if (CPU.V_Counter >= PPU.ScreenHeight + FIRST_VISIBLE_LINE) byte |= 0x80;
