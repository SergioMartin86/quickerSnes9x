...
static __thread const char *crosshairs[32] = {"`              " // Crosshair 0 (no image)
                                              "               "
                                              "               "
                                              "               "
                                              "               "
...
bool S9xLoadCrosshairFile(int idx, const char *filename)
{
    ...
    for (int r = 0; r < 15 * 15; r += 15)
    {
      ...
      for (int i = 0; i < 15; i++)
      {
        if (row_pointer[i] == transcol)
          s[r + i] = ' ';
        else if (row_pointer[i] == fgcol)
          s[r + i] = '#';
        else if (row_pointer[i] == bgcol)
          s[r + i] = '.';
        else
        {
  ...
  if (crosshairs[idx] != NULL && crosshairs[idx][0] != '`') free((void *)crosshairs[idx]);
  crosshairs[idx] = s;
