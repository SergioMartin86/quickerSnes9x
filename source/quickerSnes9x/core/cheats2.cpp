...
{
  for (unsigned int i = 0; i < sizeof(watches) / sizeof(watches[0]); i++) watches[i].on = false;
}
...
void S9xDeleteCheat(uint32 which1)
{
  ...
  {
    if (Cheat.c[which1].enabled) S9xRemoveCheat(which1);
...
void S9xRemoveCheats(void)
{
  for (uint32 i = 0; i < Cheat.num_cheats; i++)
    if (Cheat.c[i].enabled) S9xRemoveCheat(i);
}
...
void S9xApplyCheats(void)
{
  ...
  {
    for (uint32 i = 0; i < Cheat.num_cheats; i++)
      if (Cheat.c[i].enabled) S9xApplyCheat(i);
  }
}
...
bool8 S9xSaveCheatFile(const char *filename)
{
  ...
  for (uint32 i = 0; i < Cheat.num_cheats; i++)
  {
    ...
    if (!Cheat.c[i].enabled) data[0] |= 4;

    if (Cheat.c[i].saved) data[0] |= 8;
