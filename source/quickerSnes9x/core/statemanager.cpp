...
StateManager::~StateManager() { deallocate(); }
...
bool StateManager::init(size_t buffer_size)
{
  ...
  if (!(buffer = new uint64_t[buf_size])) return false;
  if (!(tmp_state = new uint32_t[state_size])) return false;
  if (!(in_state = new uint32_t[state_size])) return false;
...
void StateManager::generate_delta(const void *data)
{
  ...
  if (top_ptr == bottom_ptr) crossed = true;
  ...
  if (crossed) reassign_bottom();
}

bool StateManager::push() { return false; }
