...
static __thread const char *color_names[32] = {"Trans", "Black",   "25Grey",  "50Grey",    "75Grey", "White", "Red",    "Orange",  "Yellow",     "Green",   "Cyan",
                                               "Sky",   "Blue",    "Violet",  "MagicPink", "Purple", NULL,    "tBlack", "t25Grey", "t50Grey",    "t75Grey", "tWhite",
                                               "tRed",  "tOrange", "tYellow", "tGreen",    "tCyan",  "tSky",  "tBlue",  "tViolet", "tMagicPink", "tPurple"};

static __thread const char *speed_names[4] = {"Var", "Slow", "Med", "Fast"};

static __thread const int ptrspeeds[4] = {1, 1, 4, 8};

// Note: these should be in asciibetical order!
#define THE_COMMANDS                                                                                                                                                               \
  S(BeginRecordingMovie), S(ClipWindows), S(Debugger), S(DecEmuTurbo), S(DecFrameRate), S(DecFrameTime), S(DecTurboSpeed), S(EmuTurbo), S(EndRecordingMovie), S(ExitEmu),          \
    S(IncEmuTurbo), S(IncFrameRate), S(IncFrameTime), S(IncTurboSpeed), S(LoadFreezeFile), S(LoadMovie), S(LoadOopsFile), S(Pause), S(QuickLoad000), S(QuickLoad001),              \
    S(QuickLoad002), S(QuickLoad003), S(QuickLoad004), S(QuickLoad005), S(QuickLoad006), S(QuickLoad007), S(QuickLoad008), S(QuickLoad009), S(QuickLoad010), S(QuickSave000),      \
    S(QuickSave001), S(QuickSave002), S(QuickSave003), S(QuickSave004), S(QuickSave005), S(QuickSave006), S(QuickSave007), S(QuickSave008), S(QuickSave009), S(QuickSave010),      \
    S(Reset), S(SaveFreezeFile), S(SaveSPC), S(Screenshot), S(SeekToFrame), S(SoftReset), S(SoundChannel0), S(SoundChannel1), S(SoundChannel2), S(SoundChannel3),                  \
    S(SoundChannel4), S(SoundChannel5), S(SoundChannel6), S(SoundChannel7), S(SoundChannelsOn), S(SwapJoypads), S(ToggleBG0), S(ToggleBG1), S(ToggleBG2), S(ToggleBG3),            \
    S(ToggleEmuTurbo), S(ToggleSprites), S(ToggleTransparency)
...
static __thread const char *command_names[LAST_COMMAND + 1] = {THE_COMMANDS, NULL};
...
static int maptype(int t)
{
  switch (t)
  {
  case S9xNoMapping: return (MAP_NONE);
  ...
  case S9xButtonPort:
  case S9xButtonMulti: return (MAP_BUTTON);

  case S9xAxisJoypad:
  case S9xAxisPseudopointer:
  case S9xAxisPseudobuttons:
  case S9xAxisPort: return (MAP_AXIS);

  case S9xPointer:
  case S9xPointerPort: return (MAP_POINTER);

  default: return (MAP_UNKNOWN);
  }
}
...
{
  for (set<struct exemulti *>::iterator it = exemultis.begin(); it != exemultis.end(); it++) delete *it;
  exemultis.clear();

  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++) read_idx[i][j] = 0;
...
void S9xUnmapAllControls(void)
{
  ...
  for (int i = 0; i < (int)multis.size(); i++) free(multis[i]);
  multis.clear();

  for (int i = 0; i < NUMCTLS + 1; i++) pollmap[i].clear();
...
void S9xSetController(int port, enum controllers controller, int8 id1, int8 id2, int8 id3, int8 id4)
{
  ...
  default: fprintf(stderr, "Unknown controller type %d\n", controller); break;
  }
...
void S9xReportControllers(void)
{
  ...
  for (int port = 0; port < 2; port++)
  {
    ...
    {
    case NONE: c += sprintf(c, "<none>. "); break;
    ...
    case JOYPAD5:
    case JOYPAD6:
    case JOYPAD7: c += sprintf(c, "Pad #%d. ", (int)(newcontrollers[port] - JOYPAD0 + 1)); break;

    case MOUSE0:
    case MOUSE1: c += sprintf(c, "Mouse #%d. ", (int)(newcontrollers[port] - MOUSE0 + 1)); break;
...
char *S9xGetCommandName(s9xcommand_t command)
{
  ...
  case S9xButtonJoypad:
    ...
    c = ' ';
    if (command.button.joypad.toggle)
    {
      if (c) s += c;
      s += "Toggle";
      c = 0;
    }
    if (command.button.joypad.sticky)
    {
      if (c) s += c;
      s += "Sticky";
      c = 0;
    }
    if (command.button.joypad.turbo)
    {
      if (c) s += c;
      s += "Turbo";
      c = 0;
    ...
    c = ' ';
    if (command.button.joypad.buttons & SNES_UP_MASK)
    {
      s += c;
      s += "Up";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_DOWN_MASK)
    {
      s += c;
      s += "Down";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_LEFT_MASK)
    {
      s += c;
      s += "Left";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_RIGHT_MASK)
    {
      s += c;
      s += "Right";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_A_MASK)
    {
      s += c;
      s += "A";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_B_MASK)
    {
      s += c;
      s += "B";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_X_MASK)
    {
      s += c;
      s += "X";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_Y_MASK)
    {
      s += c;
      s += "Y";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_TL_MASK)
    {
      s += c;
      s += "L";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_TR_MASK)
    {
      s += c;
      s += "R";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_START_MASK)
    {
      s += c;
      s += "Start";
      c = '+';
    }
    if (command.button.joypad.buttons & SNES_SELECT_MASK)
    {
      s += c;
      s += "Select";
      c = '+';
  ...
  case S9xButtonSuperscope:
    ...
    c = ' ';
    if (command.button.scope.fire)
    {
      s += c;
      s += "Fire";
      c = '+';
    }
    if (command.button.scope.cursor)
    {
      s += c;
      s += "Cursor";
      c = '+';
    }
    if (command.button.scope.turbo)
    {
      s += c;
      s += "ToggleTurbo";
      c = '+';
    }
    if (command.button.scope.pause)
    {
      s += c;
      s += "Pause";
      c = '+';
  ...
  case S9xButtonJustifier:
    ...
    c = ' ';
    if (command.button.justifier.trigger)
    {
      s += c;
      s += "Trigger";
      c = '+';
    }
    if (command.button.justifier.start)
    {
      s += c;
      s += "Start";
      c = '+';
  ...
  case S9xPointer:
    ...
    c = ' ';
    if (command.pointer.aim_mouse0)
    {
      s += c;
      s += "Mouse1";
      c = '+';
    }
    if (command.pointer.aim_mouse1)
    {
      s += c;
      s += "Mouse2";
      c = '+';
    }
    if (command.pointer.aim_scope)
    {
      s += c;
      s += "Superscope";
      c = '+';
    }
    if (command.pointer.aim_justifier0)
    {
      s += c;
      s += "Justifier1";
      c = '+';
    }
    if (command.pointer.aim_justifier1)
    {
      s += c;
      s += "Justifier2";
      c = '+';
  ...
  case S9xButtonPort:
  case S9xAxisPort:
  case S9xPointerPort: return (strdup("BUG: Port should have handled this instead of calling S9xGetCommandName()"));

  case S9xNoMapping: return (strdup("None"));
  ...
  default: return (strdup("BUG: Unknown command type"));
  }

  return (strdup(s.c_str()));
}

static bool strless(const char *a, const char *b) { return (strcmp(a, b) < 0); }
...
s9xcommand_t S9xGetCommandT(const char *name)
{
  ...
  else if (!strncmp(name, "Joypad", 6))
  {
    ...
    if (!strncmp(name + 8, "Axis ", 5))
    {
      ...
      if (!strncmp(s, "Left/Right ", 11))
      {
        j = 0;
        i = 0;
        s += 11;
      } else if (!strncmp(s, "Right/Left ", 11))
      {
        j = 0;
        i = 1;
        s += 11;
      } else if (!strncmp(s, "Up/Down ", 8))
      {
        j = 1;
        i = 0;
        s += 8;
      } else if (!strncmp(s, "Down/Up ", 8))
      {
        j = 1;
        i = 1;
        s += 8;
      } else if (!strncmp(s, "Y/A ", 4))
      {
        j = 2;
        i = 0;
        s += 4;
      } else if (!strncmp(s, "A/Y ", 4))
      {
        j = 2;
        i = 1;
        s += 4;
      } else if (!strncmp(s, "X/B ", 4))
      {
        j = 3;
        i = 0;
        s += 4;
      } else if (!strncmp(s, "B/X ", 4))
      {
        j = 3;
        i = 1;
        s += 4;
      } else if (!strncmp(s, "L/R ", 4))
      {
        j = 4;
        i = 0;
        s += 4;
      } else if (!strncmp(s, "R/L ", 4))
      {
        j = 4;
        i = 1;
        s += 4;
  ...
  } else if (!strncmp(name, "Superscope ", 11))
  {
    ...
    if ((cmd.button.scope.aim_offscreen = strncmp(s, "AimOffscreen", 12) ? 0 : 1))
    {
      s += i = 12;
      if (*s == ' ')
        s++;
    ...
    if ((cmd.button.scope.fire = strncmp(s, "Fire", 4) ? 0 : 1))
    {
      s += i = 4;
      if (*s == '+') s++;
    }
    if ((cmd.button.scope.cursor = strncmp(s, "Cursor", 6) ? 0 : 1))
    {
      s += i = 6;
      if (*s == '+') s++;
    }
    if ((cmd.button.scope.turbo = strncmp(s, "ToggleTurbo", 11) ? 0 : 1))
    {
      s += i = 11;
      if (*s == '+') s++;
    }
  ...
  } else if (!strncmp(name, "Justifier", 9))
  {
    ...
    if ((cmd.button.justifier.aim_offscreen = strncmp(s, "AimOffscreen", 12) ? 0 : 1))
    {
      s += i = 12;
      if (*s == ' ')
        s++;
    ...
    if ((cmd.button.justifier.trigger = strncmp(s, "Trigger", 7) ? 0 : 1))
    {
      s += i = 7;
      if (*s == '+') s++;
    }
  ...
  } else if (!strncmp(name, "Pointer ", 8))
  {
    ...
    if ((cmd.pointer.aim_mouse0 = strncmp(s, "Mouse1", 6) ? 0 : 1))
    {
      s += i = 6;
      if (*s == '+') s++;
    }
    if ((cmd.pointer.aim_mouse1 = strncmp(s, "Mouse2", 6) ? 0 : 1))
    {
      s += i = 6;
      if (*s == '+') s++;
    }
    if ((cmd.pointer.aim_scope = strncmp(s, "Superscope", 10) ? 0 : 1))
    {
      s += i = 10;
      if (*s == '+') s++;
    }
    if ((cmd.pointer.aim_justifier0 = strncmp(s, "Justifier1", 10) ? 0 : 1))
    {
      s += i = 10;
      if (*s == '+') s++;
    }
  ...
  } else if (((name[0] == '+' && name[1] == '{') || name[0] == '{') && name[strlen(name) - 1] == '}')
  {
...
const char **S9xGetAllSnes9xCommands(void) { return (command_names); }
...
{
  for (int i = 0; i < NUMCTLS + 1; i++) pollmap[i].erase(id);
...
bool S9xMapButton(uint32 id, s9xcommand_t mapping, bool poll)
{
  ...
  if (poll)
  {
    ...
    else
    {
      switch (mapping.type)
      {
      case S9xButtonJoypad: t = JOYPAD0 + mapping.button.joypad.idx; break;

      case S9xButtonMouse: t = MOUSE0 + mapping.button.mouse.idx; break;

      case S9xButtonSuperscope: t = SUPERSCOPE; break;

      case S9xButtonJustifier: t = ONE_JUSTIFIER + mapping.button.justifier.idx; break;

      case S9xButtonCommand:
      case S9xButtonPseudopointer:
      case S9xButtonPort:
      case S9xButtonMulti: t = POLL_ALL; break;
      }
    }
  }
...
bool S9xMapPointer(uint32 id, s9xcommand_t mapping, bool poll)
{
  ...
  if (poll)
  {
    ...
    else
    {
      ...
      case S9xPointerPort: pollmap[POLL_ALL].insert(id); break;
      }
    }
  }
...
bool S9xMapAxis(uint32 id, s9xcommand_t mapping, bool poll)
{
  ...
  if (poll)
  {
    switch (mapping.type)
    {
    case S9xAxisJoypad: t = JOYPAD0 + mapping.axis.joypad.idx; break;

    case S9xAxisPseudopointer:
    case S9xAxisPseudobuttons:
    case S9xAxisPort: t = POLL_ALL; break;
    }
  }
...
void S9xSetJoypadLatch(bool latch)
{
  ...
  if (latch && !FLAG_LATCH)
  {
    ...
    for (int n = 0; n < 2; n++)
    {
      for (int j = 0; j < 2; j++) read_idx[n][j] = 0;
      ...
      case JOYPAD5:
      case JOYPAD6:
      case JOYPAD7: do_polling(i); break;

      case MOUSE0:
      case MOUSE1:
        do_polling(i);
        if (!S9xMoviePlaying()) UpdatePolledMouse(i);
        break;
...
uint8 S9xReadJOYSERn(int n)
{
  ...
  if (FLAG_LATCH)
  {
    ...
    case JOYPAD5:
    case JOYPAD6:
    case JOYPAD7: return (bits | ((joypad[i - JOYPAD0].buttons & 0x8000) ? 1 : 0));

    case MOUSE0:
    case MOUSE1:
      mouse[i - MOUSE0].buttons += 0x10;
      if ((mouse[i - MOUSE0].buttons & 0x30) == 0x30) mouse[i - MOUSE0].buttons &= 0xcf;
      return (bits);

    case SUPERSCOPE: return (bits | ((superscope.read_buttons & 0x80) ? 1 : 0));

    case ONE_JUSTIFIER:
    case TWO_JUSTIFIERS: return (bits);
  ...
  } else
  {
    ...
    default: read_idx[n][0]++; return (bits);
    }
  }
}
...
void S9xControlEOF(void)
{
  ...
  for (int n = 0; n < 2; n++)
  {
    ...
    case MOUSE1:
      ...
      if (IPPU.RenderThisFrame) S9xDrawCrosshair(S9xGetCrosshair(c->img), c->fg, c->bg, mouse[i - MOUSE0].cur_x, mouse[i - MOUSE0].cur_y);
      break;

    case SUPERSCOPE:
      if (n == 1 && !(superscope.phys_buttons & SUPERSCOPE_OFFSCREEN))
      {
        if (superscope.next_buttons & (SUPERSCOPE_FIRE | SUPERSCOPE_CURSOR)) DoGunLatch(superscope.x, superscope.y);

        c = &superscope.crosshair;
        if (IPPU.RenderThisFrame) S9xDrawCrosshair(S9xGetCrosshair(c->img), c->fg, c->bg, superscope.x, superscope.y);
      }
    ...
    case TWO_JUSTIFIERS:
      if (n == 1 && !justifier.offscreen[1])
      {
        ...
        if (IPPU.RenderThisFrame) S9xDrawCrosshair(S9xGetCrosshair(c->img), c->fg, c->bg, justifier.x[1], justifier.y[1]);
      }
    ...
    do_justifier:
      if (n == 1)
      {
        if (i >= 0 && !justifier.offscreen[i]) DoGunLatch(justifier.x[i], justifier.y[i]);

        if (!justifier.offscreen[0])
        {
          c = &justifier.crosshair[0];
          if (IPPU.RenderThisFrame) S9xDrawCrosshair(S9xGetCrosshair(c->img), c->fg, c->bg, justifier.x[0], justifier.y[0]);
        }
      }
...
void S9xGetControllerCrosshair(enum crosscontrols ctl, int8 *idx, const char **fg, const char **bg)
{
  ...
  default: fprintf(stderr, "S9xGetControllerCrosshair() called with an invalid controller ID %d\n", ctl); return;
  }

  if (idx) *idx = c->img;

  if (fg) *fg = color_names[c->fg];

  if (bg) *bg = color_names[c->bg];
}
...
void S9xControlPreSaveState(struct SControlSnapshot *s)
{
  ...
  for (int j = 0; j < 2; j++) s->mouse_speed[j] = (mouse[j].buttons & 0x30) >> 4;

  s->justifier_select = ((justifier.buttons & JUSTIFIER_SELECT) ? 1 : 0);

#define COPY(x)                                                                                                                                                                    \
  {                                                                                                                                                                                \
    memcpy((char *)s->internal + i, &(x), sizeof(x));                                                                                                                              \
    i += sizeof(x);                                                                                                                                                                \
  }

  int i = 0;

  for (int j = 0; j < 8; j++) COPY(joypad[j].buttons);
  ...
  for (int j = 0; j < 2; j++) COPY(justifier.x[j]);
  for (int j = 0; j < 2; j++) COPY(justifier.y[j]);
  COPY(justifier.buttons);
  for (int j = 0; j < 2; j++) COPY(justifier.offscreen[j]);

  for (int j = 0; j < 2; j++)
    for (int k = 0; k < 2; k++) COPY(mp5[j].pads[k]);
...
void S9xControlPostLoadState(struct SControlSnapshot *s)
{
  ...
  for (int j = 0; j < 2; j++) mouse[j].buttons |= (s->mouse_speed[j] & 3) << 4;
...
#define COPY(x)                                                                                                                                                                    \
  {                                                                                                                                                                                \
    memcpy(&(x), (char *)s->internal + i, sizeof(x));                                                                                                                              \
    i += sizeof(x);                                                                                                                                                                \
  }
...
bool MovieGetMouse(int i, uint8 out[5])
{
  ...
  WRITE_WORD(ptr, mouse[n].cur_x);
  ptr += 2;
  WRITE_WORD(ptr, mouse[n].cur_y);
  ptr += 2;
  *ptr = mouse[n].buttons;
...
void MovieSetMouse(int i, uint8 in[5], bool inPolling)
{
  ...
  mouse[n].cur_x = READ_WORD(ptr);
  ptr += 2;
  mouse[n].cur_y = READ_WORD(ptr);
  ptr += 2;
  mouse[n].buttons = *ptr;

  if (inPolling) UpdatePolledMouse(curcontrollers[i]);
}
...
bool MovieGetScope(int i, uint8 out[6])
{
  ...
  WRITE_WORD(ptr, superscope.x);
  ptr += 2;
  WRITE_WORD(ptr, superscope.y);
  ptr += 2;
...
void MovieSetScope(int i, uint8 in[6])
{
  ...
  superscope.x = READ_WORD(ptr);
  ptr += 2;
  superscope.y = READ_WORD(ptr);
  ptr += 2;
...
bool MovieGetJustifier(int i, uint8 out[11])
{
  ...
  WRITE_WORD(ptr, justifier.x[0]);
  ptr += 2;
  WRITE_WORD(ptr, justifier.x[1]);
  ptr += 2;
  WRITE_WORD(ptr, justifier.y[0]);
  ptr += 2;
  WRITE_WORD(ptr, justifier.y[1]);
  ptr += 2;
...
void MovieSetJustifier(int i, uint8 in[11])
{
  ...
  justifier.x[0] = READ_WORD(ptr);
  ptr += 2;
  justifier.x[1] = READ_WORD(ptr);
  ptr += 2;
  justifier.y[0] = READ_WORD(ptr);
  ptr += 2;
  justifier.y[1] = READ_WORD(ptr);
  ptr += 2;
