...
static void Write8(uint8 v, uint8 *&ptr) { *ptr++ = v; }
...
static int bytes_per_sample(void)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    if (Movie.ControllersMask & (1 << i)) num_controllers++;
  }
  ...
  for (int p = 0; p < 2; p++)
  {
    ...
    else if (Movie.PortType[p] == CTL_SUPERSCOPE)
      bytes += SCOPE_DATA_SIZE;
    else if (Movie.PortType[p] == CTL_JUSTIFIER)
...
static void reset_controllers(void)
{
  for (int i = 0; i < 8; i++) MovieSetJoypad(i, 0);
...
static void write_movie_header(FILE *fd, SMovie *movie)
{
  ...
  for (int p = 0; p < 2; p++)
  {
    for (int i = 0; i < 4; i++) Write8(movie->PortIDs[p][i], ptr);
  }
...
{
  if (new_state == Movie.State) return;

  if (Movie.State == MOVIE_STATE_RECORD) flush_movie();
  ...
  if (new_state == MOVIE_STATE_NONE)
  {
    ...
    if (S9xMoviePlaying() || S9xMovieRecording()) restore_previous_settings();
  }
...
{
  if (!S9xMovieActive()) return (FILE_NOT_FOUND);

  if (size < sizeof(Movie.MovieId) + sizeof(Movie.CurrentFrame) + sizeof(Movie.MaxFrame) + sizeof(Movie.CurrentSample) + sizeof(Movie.MaxSample)) return (WRONG_FORMAT);
  ...
  if (current_frame > max_frame || current_sample > max_sample || space_needed > size) return (WRONG_MOVIE_SNAPSHOT);
  ...
  } else
  {
    ...
    if (current_frame > Movie.MaxFrame || current_sample > Movie.MaxSample || memcmp(Movie.InputBuffer, ptr, space_processed)) return (SNAPSHOT_INCONSISTENT);
...
int S9xMovieCreate(const char *filename, uint8 controllers_mask, uint8 opts, const wchar_t *metadata, int metadata_length)
{
  ...
  if (controllers_mask == 0) return (WRONG_FORMAT);

  if (!(fd = fopen(filename, "wb"))) return (FILE_NOT_FOUND);

  if (metadata_length > MOVIE_MAX_METADATA) metadata_length = MOVIE_MAX_METADATA;
  ...
  if (!stream) return (FILE_NOT_FOUND);
  ...
  if (!(fd = fopen(filename, "rb+"))) return (FILE_NOT_FOUND);
...
int S9xMovieGetInfo(const char *filename, struct MovieInfo *info) { return (SUCCESS); }
...
void S9xMovieUpdate(bool addFrame)
{
  ...
  case MOVIE_STATE_PLAY:
  {
    ...
    } else
    {
      ...
      Movie.CurrentSample++;
      if (addFrame) Movie.CurrentFrame++;
    }
  ...
  default:
  {
    if (addFrame) S9xUpdateFrameCounter();
...
void S9xMovieStop(bool8 suppress_message)
{
  if (Movie.State != MOVIE_STATE_NONE)
  {
    ...
    if (!suppress_message) S9xMessage(S9X_INFO, S9X_MOVIE_INFO, MOVIE_INFO_STOP);
  }
}

void S9xMovieShutdown(void)
{
  if (S9xMovieActive()) S9xMovieStop(TRUE);
}

bool8 S9xMovieActive(void) { return (Movie.State != MOVIE_STATE_NONE); }

bool8 S9xMoviePlaying(void) { return (Movie.State == MOVIE_STATE_PLAY); }

bool8 S9xMovieRecording(void) { return (Movie.State == MOVIE_STATE_RECORD); }

uint8 S9xMovieControllers(void) { return (Movie.ControllersMask); }

bool8 S9xMovieReadOnly(void)
{
  if (!S9xMovieActive()) return (FALSE);
