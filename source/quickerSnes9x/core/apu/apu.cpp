...
static void EightBitize(uint8 *buffer, int sample_count)
{
  ...
  for (int i = 0; i < sample_count; i++) buf8[i] = (uint8)((buf16[i] / 256) + 128);
}
...
bool8 S9xMixSamples(uint8 *buffer, int sample_count)
{
  ...
  if (Settings.Mute)
  {
    ...
    if (Settings.MSU1) msu::resampler->clear();
  ...
  } else
  {
    if (spc::resampler->avail() >= (sample_count + spc::lag))
    {
      ...
      if (spc::lag == spc::lag_master) spc::lag = 0;
      ...
      if (Settings.MSU1)
      {
        if (msu::resampler->avail() >= sample_count)
        {
          ...
          for (int32 i = 0; i < sample_count; ++i) *((int16 *)(dest + (i * 2))) += *((int16 *)(msu_sample + (i * 2)));
        }
      }
    } else
    {
      memset(buffer, (Settings.SixteenBitSound ? 0 : 128), (sample_count << (Settings.SixteenBitSound ? 1 : 0)) >> (Settings.Stereo ? 0 : 1));
      if (spc::lag == 0) spc::lag = spc::lag_master;

      return (FALSE);
    }
  }

  if (Settings.ReverseStereo && Settings.Stereo) ReverseStereo(dest, sample_count);
  ...
  if (!Settings.Stereo || !Settings.SixteenBitSound)
  {
    ...
    if (!Settings.SixteenBitSound) EightBitize(dest, sample_count);
...
int S9xGetSampleCount(void) { return (spc::resampler->avail() >> (Settings.Stereo ? 0 : 1)); }
...
void S9xFinalizeSamples(void)
{
  if (!Settings.Mute)
  {
    ...
    if (!spc::resampler->push((short *)spc::landing_buffer, SNES::dsp.spc_dsp.sample_count()))
    {
      ...
      if (Settings.SoundSync && !Settings.TurboMode) return;
    }
  }
  ...
  if (Settings.MSU1) S9xMSU1SetOutput((int16 *)msu::landing_buffer, msu::buffer_size);
}
...
{
  if (!Settings.SoundSync || spc::sound_in_sync) return (TRUE);
...
static void UpdatePlaybackRate(void)
{
  if (Settings.SoundInputRate == 0) Settings.SoundInputRate = APU_DEFAULT_INPUT_RATE;
...
bool8 S9xInitSound(int buffer_ms, int lag_ms)
{
  ...
  if (Settings.Stereo) spc::lag_master <<= 1;
  spc::lag = spc::lag_master;

  if (sample_count < APU_MINIMUM_SAMPLE_COUNT) sample_count = APU_MINIMUM_SAMPLE_COUNT;

  spc::buffer_size = sample_count;
  if (Settings.Stereo) spc::buffer_size <<= 1;
  if (Settings.SixteenBitSound) spc::buffer_size <<= 1;
  msu::buffer_size = ((buffer_ms * 44100 / 1000) * 44100 / 32040) << 2; // Always 16-bit, Stereo

  //	printf("Sound buffer size: %d (%d samples)\n", spc::buffer_size, sample_count);

  if (spc::landing_buffer) delete[] spc::landing_buffer;
  spc::landing_buffer = new uint8[spc::buffer_size * 2];
  if (!spc::landing_buffer) return (FALSE);
  if (msu::landing_buffer) delete[] msu::landing_buffer;
  msu::landing_buffer = new uint8[msu::buffer_size * 2];
  if (!msu::landing_buffer) return (FALSE);
...
void S9xSetSoundControl(uint8 voice_switch) { SNES::dsp.spc_dsp.set_stereo_switch(voice_switch << 8 | voice_switch); }

void S9xSetSoundMute(bool8 mute)
{
  Settings.Mute = mute;
  if (!spc::sound_enabled) Settings.Mute = TRUE;
}

void S9xDumpSPCSnapshot(void) { SNES::dsp.spc_dsp.dump_spc_snapshot(); }
...
static inline int S9xAPUGetClock(int32 cpucycles) { return (spc::ratio_numerator * (cpucycles - spc::reference_time) + spc::remainder) / spc::ratio_denominator; }

static inline int S9xAPUGetClockRemainder(int32 cpucycles) { return (spc::ratio_numerator * (cpucycles - spc::reference_time) + spc::remainder) % spc::ratio_denominator; }
...
void S9xAPUSetReferenceTime(int32 cpucycles) { spc::reference_time = cpucycles; }
...
void S9xAPUEndScanline(void)
{
  ...
  if (SNES::dsp.spc_dsp.sample_count() >= APU_MINIMUM_SAMPLE_BLOCK || !spc::sound_in_sync) S9xLandSamples();
}

void S9xAPUTimingSetSpeedup(int ticks)
{
  if (ticks != 0) printf("APU speedup hack: %d\n", ticks);
...
bool8 S9xSPCDump(const char *filename)
{
  ...
  if (ignore == 0) { fprintf(stderr, "Couldn't write file %s.\n", filename); }
