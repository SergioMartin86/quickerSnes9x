...
class SPC_DSP
{
  ...
  {
    state_size = 640
  }; // maximum space needed when saving
...
inline void SPC_DSP::write(int addr, int data)
{
  ...
  switch (addr & 0x0F)
  {
  case v_envx: m.envx_buf = (uint8_t)data; break;

  case v_outx: m.outx_buf = (uint8_t)data; break;

  case 0x0C:
    if (addr == r_kon) m.new_kon = (uint8_t)data;
...
class SPC_State_Copier
{
  ...
  SPC_State_Copier(unsigned char **p, SPC_DSP::copy_func_t f)
  {
    func = f;
    buf  = p;
  }
