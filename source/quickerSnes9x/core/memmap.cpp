...
// from NSRT
static __thread const char *nintendo_licensees[] = {"Unlicensed",
                                                    "Nintendo",
...
static void S9xDeinterleaveType2(int size, uint8 *base)
{
  ...
  while (nblocks <= step) step >>= 1;
  nblocks = step;

  for (int i = 0; i < nblocks * 2; i++) blocks[i] = (i & ~0xf) | ((i & 3) << 2) | ((i & 12) >> 2);
...
bool8 CMemory::Init(void)
{
  ...
  if (!RAM || !SRAM || !VRAM || !ROM || !IPPU.TileCache[TILE_2BIT] || !IPPU.TileCache[TILE_4BIT] || !IPPU.TileCache[TILE_8BIT] || !IPPU.TileCache[TILE_2BIT_EVEN] ||
      !IPPU.TileCache[TILE_2BIT_ODD] || !IPPU.TileCache[TILE_4BIT_EVEN] || !IPPU.TileCache[TILE_4BIT_ODD] || !IPPU.TileCached[TILE_2BIT] || !IPPU.TileCached[TILE_4BIT] ||
      !IPPU.TileCached[TILE_8BIT] || !IPPU.TileCached[TILE_2BIT_EVEN] || !IPPU.TileCached[TILE_2BIT_ODD] || !IPPU.TileCached[TILE_4BIT_EVEN] || !IPPU.TileCached[TILE_4BIT_ODD])
  {
    Deinit();
    return (FALSE);
  }
...
{
  if (size == 0x40000 && strncmp((char *)data, "BANDAI SFC-ADX", 14) == 0 && strncmp((char *)(data + 0x10), "SFC-ADX BACKUP", 14) == 0)
    return (TRUE);
  else
    return (FALSE);
}
...
{
  if (size >= 0x80000 && size <= 0x100000 && strncmp((char *)data, "BANDAI SFC-ADX", 14) == 0 && strncmp((char *)(data + 0x10), "SFC-ADX BACKUP", 14) != 0)
    return (TRUE);
  else
    return (FALSE);
}
...
static bool8 is_BSCartSA1_BIOS(const uint8 *data, uint32 size)
{
  ...
  if (!is_BSCart_BIOS(data, size)) return (FALSE);
...
int CMemory::ScoreHiROM(bool8 skip_header, int32 romoff)
{
  ...
  {
    score += 2;
    if (0 != (buf[0xde] + (buf[0xdf] << 8))) score++;
  }
  ...
  if (CalculatedSize > 1024 * 1024 * 3) score += 4;

  if ((1 << (buf[0xd7] - 7)) > 48) score -= 1;

  if (!allASCII(&buf[0xb0], 6)) score -= 1;

  if (!allASCII(&buf[0xc0], ROM_NAME_LEN - 1)) score -= 1;
...
int CMemory::ScoreLoROM(bool8 skip_header, int32 romoff)
{
  ...
  if ((buf[0xfc] + (buf[0xfd] << 8)) > 0xffb0) score -= 2; // reduced per Cowering suggestion

  if (CalculatedSize <= 1024 * 1024 * 16) score += 2;
...
uint32 CMemory::HeaderRemove(uint32 size, uint8 *buf)
{
  ...
  if ((size - calc_size == 512 && !Settings.ForceNoHeader) || Settings.ForceHeader)
  {
    ...
    if (!strncmp("NSRT", (char *)&NSRTHead[24], 4))
    {
      if (NSRTHead[28] == 22)
      {
        if (((std::accumulate(NSRTHead, NSRTHead + sizeof(NSRTHeader), 0) & 0xFF) == NSRTHead[30]) && (NSRTHead[30] + NSRTHead[31] == 255) && ((NSRTHead[0] & 0x0F) <= 13) &&
  ...
  default:
  {
    ...
    do {
      ...
      if (ptr - buffer < maxsize + 0x200 && (isdigit(ext[0]) && ext[1] == 0 && ext[0] < '9'))
      {
        ...
        ext[0]++;
        _makepath(fname, drive, dir, name, exts);
      } else if (ptr - buffer < maxsize + 0x200 && (((len = strlen(name)) == 7 || len == 8) && strncasecmp(name, "sf", 2) == 0 && isdigit(name[2]) && isdigit(name[3]) &&
                                                    isdigit(name[4]) && isdigit(name[5]) && isalpha(name[len - 1])))
      {
        more = TRUE;
        name[len - 1]++;
        _makepath(fname, drive, dir, name, exts);
...
{
  if (!source || sourceSize > MAX_ROM_SIZE) return FALSE;
...
bool8 CMemory::LoadROM(const char *filename)
{
  ...
  do {
    ...
    if (!totalFileSize) return (FALSE);

    if (!Settings.NoPatch) CheckForAnyPatch(filename, HeaderCount != 0, totalFileSize);
  }
...
bool8 CMemory::LoadROMInt(int32 ROMfillSize)
{
  ...
  if (HeaderCount == 0 && !Settings.ForceNoHeader && ((hi_score > lo_score && ScoreHiROM(TRUE) > hi_score) || (hi_score <= lo_score && ScoreLoROM(TRUE) > lo_score)))
  {
  ...
  if (CalculatedSize > 0x400000 && (ROM[0x7fd5] + (ROM[0x7fd6] << 8)) != 0x3423 &&                    // exclude SA-1
      (ROM[0x7fd5] + (ROM[0x7fd6] << 8)) != 0x3523 && (ROM[0x7fd5] + (ROM[0x7fd6] << 8)) != 0x4332 && // exclude S-DD1
      (ROM[0x7fd5] + (ROM[0x7fd6] << 8)) != 0x4532 && (ROM[0xffd5] + (ROM[0xffd6] << 8)) != 0xF93a && // exclude SPC7110
  ...
  {
    if (!Settings.ForceInterleaved && !Settings.ForceNotInterleaved) S9xDeinterleaveType1(ROMfillSize, ROM);
  }
  ...
  {
    if (strncmp((char *)&ROM[0x7fc0], "YUYU NO QUIZ DE GO!GO!", 22) == 0 || (strncmp((char *)&ROM[0xffc0], "BATMAN--REVENGE JOKER", 21) == 0))
    {
      LoROM       = TRUE;
      HiROM       = FALSE;
      interleaved = FALSE;
  ...
  if (ExtendedFormat == SMALLFIRST) tales = TRUE;
...
bool8 CMemory::LoadMultiCartMem(const uint8 *sourceA, uint32 sourceASize, const uint8 *sourceB, uint32 sourceBSize, const uint8 *bios, uint32 biosSize)
{
  ...
  if (bios)
  {
    if (!is_SufamiTurbo_BIOS(bios, biosSize)) return FALSE;
...
bool8 CMemory::LoadMultiCart(const char *cartA, const char *cartB)
{
  ...
  if (cartB && cartB[0]) Multi.cartSizeB = FileLoader(ROM, cartB, MAX_ROM_SIZE);

  if (Multi.cartSizeB)
  {
    strcpy(Multi.fileNameB, cartB);

    if (!Settings.NoPatch) CheckForAnyPatch(cartB, HeaderCount != 0, Multi.cartSizeB);

    Multi.cartOffsetB = 0x400000;
    memcpy(ROM + Multi.cartOffsetB, ROM, Multi.cartSizeB);
  }

  if (cartA && cartA[0]) Multi.cartSizeA = FileLoader(ROM, cartA, MAX_ROM_SIZE);

  if (Multi.cartSizeA)
  {
    strcpy(Multi.fileNameA, cartA);

    if (!Settings.NoPatch) CheckForAnyPatch(cartA, HeaderCount != 0, Multi.cartSizeA);
  }
...
bool8 CMemory::LoadMultiCartInt()
{
  ...
  {
    if (is_SufamiTurbo_Cart(ROM + Multi.cartOffsetB, Multi.cartSizeB)) Multi.cartType = 4;
  } else
    Multi.cartType = 4; // assuming BIOS only

  if (Multi.cartType == 4 && Multi.cartOffsetA == 0)
  { // try to load bios from file
    ...
    if (fp)
    {
      ...
      fclose(fp);
      if (!is_SufamiTurbo_BIOS(ROM, size)) return (FALSE);
    } else
      return (FALSE);
  ...
  {
  case 4: r = LoadSufamiTurbo(); break;

  case 3:
  case 5: r = LoadBSCart(); break;
...
bool8 CMemory::LoadSufamiTurbo()
{
  ...
  if (Multi.cartSizeB)
  {
    if (!is_SufamiTurbo_Cart(ROM + Multi.cartOffsetB, Multi.cartSizeB)) Multi.cartSizeB = 0;
  }
...
bool8 CMemory::LoadBSCart()
{
  ...
  if (Multi.cartSizeB == 0 && (int64_t)Multi.cartSizeA <= (int64_t)(MAX_ROM_SIZE - 0x100000 - Multi.cartOffsetA))
  {
...
bool8 CMemory::LoadGNEXT()
{
  ...
  if (Multi.cartSizeB)
  {
    if (!is_GNEXT_Add_On(ROM + Multi.cartOffsetB, Multi.cartSizeB)) Multi.cartSizeB = 0;
  }
...
bool8 CMemory::LoadSRTC(void)
{
  ...
  int read = fread(RTCData.reg, 1, 20, fp);
...
bool8 CMemory::LoadSRAM(const char *filename)
{
  ...
  if (Multi.cartType && Multi.sramSizeB)
  {
    ...
    if (file)
    {
      ...
      fclose(file);
      if (len - size == 512) memmove(Multi.sramB, Multi.sramB + 512, size);
    }
  ...
  if (size)
  {
    ...
    if (file)
    {
      ...
      fclose(file);
      if (len - size == 512) memmove(SRAM, SRAM + 512, size);

      if (Settings.SRTC || Settings.SPC7110RTC) LoadSRTC();
...
bool8 CMemory::SaveSRAM(const char *filename)
{
  ...
  if (size)
  {
    ...
    if (file)
    {
      ...
      if (Settings.SRTC || Settings.SPC7110RTC) SaveSRTC();
...
{
  for (uint32 i = 0; i < size; i++) crc32 = ((crc32 >> 8) & 0x00FFFFFF) ^ crc32Table[(crc32 ^ array[i]) & 0xFF];
...
char *CMemory::SafeANK(const char *s)
{
  ...
  for (int i = 0; i < len; i++)
  {
    if (s[i] >= 32 && s[i] < 127) // ASCII
      safe[i] = s[i];
    else if (ROMRegion == 0 && ((uint8)s[i] >= 0xa0 && (uint8)s[i] < 0xe0)) // JIS X 201 - Katakana
      safe[i] = s[i];
    else
      safe[i] = '_';
  }
...
void CMemory::ParseSNESHeader(uint8 *RomHeader)
{
  ...
  if (bs) memset(ROMName + 16, 0x20, ROM_NAME_LEN - 17);

  if (bs)
  {
    if (!(((RomHeader[0x29] & 0x20) && CalculatedSize < 0x100000) || (!(RomHeader[0x29] & 0x20) && CalculatedSize == 0x100000))) printf("BS: Size mismatch\n");

    // FIXME
    int p = 0;
    while ((1 << p) < (int)CalculatedSize) p++;
    ROMSize = p - 10;
  } else
...
void CMemory::InitROM(void)
{
  ...
  if (ExtendedFormat == BIGFIRST) RomHeader += 0x400000;
  if (HiROM) RomHeader += 0x8000;
  ...
  // SPC7110
  case 0xF93A: Settings.SPC7110RTC = TRUE;
  case 0xF53A:
  ...
  // OBC1
  case 0x2530: Settings.OBC1 = TRUE; break;

  // SA1
  case 0x3423:
  case 0x3523: Settings.SA1 = TRUE; break;
  ...
  // SDD1
  case 0x4332:
  case 0x4532: Settings.SDD1 = TRUE; break;
  ...
  // C4
  case 0xF320: Settings.C4 = TRUE; break;
  }
  ...
  sprintf(String,
          "\"%s\" [%s] %s, %s, %s, %s, SRAM:%s, ID:%s, CRC32:%08X",
          displayName,
          isChecksumOK ? "checksum ok" : ((Multi.cartType == 4) ? "no checksum" : "bad checksum"),
          MapType(),
          Size(),
          KartContents(),
  ...
  if (stopMovie) S9xMovieStop(TRUE);

  if (PostRomInitFunc) PostRomInitFunc();
...
void CMemory::map_WriteProtectROM(void)
{
  ...
  {
    if (BlockIsROM[c]) WriteMap[c] = (uint8 *)MAP_NONE;
  }
}
...
void CMemory::Map_SA1LoROMMap(void)
{
  ...
  for (int c = 0x40; c < 0x80; c++) map_space(c, c, 0x0000, 0xffff, SRAM + (c & 1) * 0x10000);
  ...
  // SA-1 Banks 60->6f
  for (int c = 0x600; c < 0x700; c++) SA1.Map[c] = SA1.WriteMap[c] = (uint8 *)MAP_BWRAM_BITMAP;
...
uint16 CMemory::checksum_calc_sum(uint8 *data, uint32 length)
{
  ...
  for (uint32 i = 0; i < length; i++) sum += data[i];
...
void CMemory::Checksum_Calculate(void)
{
  ...
  else if (Settings.SPC7110)
  {
    ...
    if (CalculatedSize == 0x300000) sum += sum;
  } else
  {
...
const char *CMemory::MapType(void) { return (HiROM ? ((ExtendedFormat != NOPE) ? "ExHiROM" : "HiROM") : "LoROM"); }
...
const char *CMemory::KartContents(void)
{
  ...
  if (ROMType == 0 && !Settings.BS) return ("ROM");
  ...
  if (Settings.MSU1) sprintf(chip + strlen(chip), "+MSU-1");
...
{
  if (CompanyId >= (int)(sizeof(nintendo_licensees) / sizeof(nintendo_licensees[0])) || CompanyId < 0) return ("Unknown");

  if (nintendo_licensees[CompanyId] == NULL) return ("Unknown");
...
bool8 CMemory::match_na(const char *str) { return (strcmp(ROMName, str) == 0); }

bool8 CMemory::match_nn(const char *str) { return (strncmp(ROMName, str, strlen(str)) == 0); }

bool8 CMemory::match_nc(const char *str) { return (strncasecmp(ROMName, str, strlen(str)) == 0); }

bool8 CMemory::match_id(const char *str) { return (strncmp(ROMId, str, strlen(str)) == 0); }
...
void CMemory::ApplyROMFixes(void)
{
  ...
  if ((ROMCRC32 == 0x6810aa95) || (ROMCRC32 == 0x340f23e5) || (ROMCRC32 == 0x77fd806a) || (match_nn("HIGHWAY BATTLE 2")) ||
      (match_na("FX SKIING NINTENDO 96") && (ROM[0x7fda] == 0)) || (match_nn("HONKAKUHA IGO GOSEI") && (ROM[0xffd5] != 0x31)))
  {
  ...
  if (!Settings.DisableGameSpecificHacks)
  {
    ...
    if (match_na("SUPER DRIFT OUT") ||                            // Super Drift Out
        match_na("SATAN IS OUR FATHER!") || match_na("goemon 4")) // Ganbare Goemon Kirakira Douchuu
      SNESGameFixes.SRAMInitialValue = 0x00;
...
static bool8 ReadUPSPatch(Stream *r, long, int32 &rom_size) { return false; }

// header notes for UPS patches also apply to BPS
//
// logic taken from http://byuu.org/programming/bps and the accompanying source
//
static bool8 ReadBPSPatch(Stream *r, long, int32 &rom_size) { return false; }
...
static bool8 ReadIPSPatch(Stream *r, long offset, int32 &rom_size)
{
  ...
  if (strncmp(fname, "PATCH", 5)) return (0);
  ...
  for (;;)
  {
    ...
    if (len)
    {
      if (ofs + len > CMemory::MAX_ROM_SIZE) return (0);

      while (len--)
      {
        rchar = r->get_char();
        if (rchar == EOF) return (0);
        Memory.ROM[ofs++] = (uint8)rchar;
      }

      if (ofs > rom_size) rom_size = ofs;
    } else
    {
      rlen = ReadInt(r, 2);
      if (rlen == -1) return (0);
      ...
      while (rlen--) Memory.ROM[ofs++] = (uint8)rchar;
  ...
  ofs = ReadInt(r, 3);
  if (ofs != -1 && ofs - offset < rom_size) rom_size = ofs - offset;
...
static int unzFindExtension(unzFile &file, const char *ext, bool restart, bool print)
{
  ...
  while (port == UNZ_OK)
  {
    ...
    {
      if (print) printf("Using patch %s", name);
...
void CMemory::CheckForAnyPatch(const char *rom_filename, bool8 header, int32 &rom_size)
{
  ...
  if (_MAX_EXT > 6)
  {
    ...
    do {
      ...
      if (!(patch_file = OPEN_FSTREAM(fname, "rb"))) break;
  ...
  if (_MAX_EXT > 6)
  {
    ...
    do {
      ...
      if (!(patch_file = OPEN_FSTREAM(n, "rb"))) break;
